#░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░▒▓████████▓▒░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░ 
#░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░ 
#░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░ 
#░▒▓██████▓▒░  ░▒▓██████▓▒░░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓██████▓▒░  ░▒▓██████▓▒░  
#░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░         ░▒▓█▓▒░     
#░▒▓█▓▒░      ░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░         ░▒▓█▓▒░     
#░▒▓████████▓▒░▒▓█▓▒░░▒▓█▓▒░▒▓█▓▒░  ░▒▓█▓▒░   ░▒▓█▓▒░         ░▒▓█▓▒░  
#если берете код, укажите оригинальный источник.
__id__ = "exitfy"
__name__ = "exitFy"
__description__ = "Application download speed optimization"
__icon__ = "exitFy/1"
__author__ = "@exteraPluginsSup"
__version__ = "2.3.1"
__min_version__ = "12.1.1"

import threading, time, requests, urllib.parse, socket, ctypes, base64, json, re, random
import os
import hashlib
import urllib.request
import urllib.error
import uuid
from html import unescape

try:
    import ssl
except Exception:
    ssl = None

try:
    import struct
except Exception:
    struct = None

from base_plugin import BasePlugin, MenuItemData, MenuItemType, MethodHook
from android_utils import log, run_on_ui_thread
from android.graphics import Color
from ui.settings import Header, Switch, Selector, Text, Divider, Input
from ui.bulletin import BulletinHelper
from ui.alert import AlertDialogBuilder
from hook_utils import find_class, get_private_field, set_private_field
from client_utils import get_last_fragment
from java.util import Locale
from android.os import Build
from android.provider import Settings
from android.content import Context
from android.net import NetworkCapabilities
from org.telegram.messenger import (
    ApplicationLoader, SharedConfig, MessagesController,
    NotificationCenter, UserConfig,
)
from org.telegram.tgnet import ConnectionsManager

try:
    from java.lang import Integer
    from javax.crypto import Cipher as JavaCipher
    from javax.crypto.spec import SecretKeySpec, IvParameterSpec
except Exception:
    Integer = None
    JavaCipher = None
    SecretKeySpec = None
    IvParameterSpec = None

try:
    from java import dynamic_proxy
except Exception:
    dynamic_proxy = None

try:
    from java import jclass
except Exception:
    jclass = None

MKSTATS_API_URL = os.getenv("MKSTATS_API_URL", "https://mkstats.mk69.su/api")
MKSTATS_PING_INTERVAL = int(os.getenv("MKSTATS_PING_INTERVAL", "1500"))
MKSTATS_POW_SOLVE_SECONDS = int(os.getenv("MKSTATS_POW_SOLVE_SECONDS", "6"))

EXITFY_VERBOSE = str(os.getenv("EXITFY_VERBOSE", "0") or "0").strip().lower() in ("1", "true", "yes", "y", "on")

def generate_user_hash(device_id: str, plugin_id: str) -> str:
    payload = f"{device_id}:{plugin_id}:mkstats:v1"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()

def generate_device_fingerprint(device_id: str) -> str:
    payload = f"{device_id}:mkstats:device:v1"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()

def _normalize_api_base(api_url: str) -> str:
    base = api_url.rstrip("/")
    if base.endswith("/api"):
        return f"{base}/v1"
    return base

def _post_json(url: str, payload: dict) -> dict:
    data = json.dumps(payload).encode("utf-8")
    request = urllib.request.Request(
        url, data=data, headers={"Content-Type": "application/json"}
    )
    try:
        with urllib.request.urlopen(request, timeout=10) as response:
            body = response.read().decode("utf-8")
            try:
                out = json.loads(body)
            except Exception:
                out = {"raw": body}
            try:
                out["_http_status"] = int(getattr(response, "status", 200) or 200)
            except Exception:
                out["_http_status"] = 200
            return out
    except urllib.error.HTTPError as e:
        try:
            body = e.read().decode("utf-8")
        except Exception:
            body = ""
        try:
            out = json.loads(body) if body else {}
        except Exception:
            out = {"raw": body}
        try:
            out["_http_status"] = int(getattr(e, "code", 0) or 0)
        except Exception:
            out["_http_status"] = 0
        return out
    except Exception as e:
        return {"_http_status": 0, "error": str(e)}

def _pow_valid(challenge: str, nonce: str, difficulty: int) -> bool:
    if not challenge or not nonce or difficulty <= 0:
        return False
    prefix = "0" * max(1, int(difficulty))
    digest = hashlib.sha256(f"{challenge}:{nonce}".encode("utf-8")).hexdigest()
    return digest.startswith(prefix)

def _solve_pow(challenge: str, difficulty: int, max_seconds: int = MKSTATS_POW_SOLVE_SECONDS) -> str | None:
    difficulty = max(1, int(difficulty or 0))
    deadline = time.time() + max(1, int(max_seconds or 0))
    nonce = 0
    prefix = "0" * difficulty
    while time.time() < deadline:
        candidate = format(nonce, "x")
        digest = hashlib.sha256(f"{challenge}:{candidate}".encode("utf-8")).hexdigest()
        if digest.startswith(prefix):
            return candidate
        nonce += 1
    return None

class MkStatsCoreClient:
    def __init__(self, api_url: str, plugin_id: str, plugin_version: str, user_hash: str, device_fingerprint: str, client_version: str | None = None, client_name: str | None = None) -> None:
        self.api_base = _normalize_api_base(api_url)
        self.plugin_id = plugin_id
        self.plugin_version = plugin_version
        self.client_version = client_version
        self.client_name = client_name
        self.user_hash = user_hash
        self.device_fingerprint = device_fingerprint

    def handshake(self) -> dict:
        payload = {
            "plugin_id": self.plugin_id,
            "version": self.plugin_version,
            "client_name": self.client_name,
            "client_version": self.client_version,
            "user_hash": self.user_hash,
            "device_fingerprint": self.device_fingerprint,
        }
        response = _post_json(f"{self.api_base}/handshake", payload)
        token = (response or {}).get("install_token", "")
        if token:
            return response
        pow_required = bool((response or {}).get("pow_required"))
        pow_challenge = (response or {}).get("pow_challenge")
        if pow_required and pow_challenge:
            difficulty = int((response or {}).get("pow_difficulty") or 0)
            nonce = _solve_pow(pow_challenge, difficulty)
            if nonce and _pow_valid(pow_challenge, nonce, difficulty):
                payload["pow_challenge"] = pow_challenge
                payload["pow_nonce"] = nonce
                response = _post_json(f"{self.api_base}/handshake", payload)
        return response

    def send_ping(self, install_token: str, timestamp=None) -> dict:
        payload = {
            "plugin_id": self.plugin_id,
            "version": self.plugin_version,
            "client_name": self.client_name,
            "client_version": self.client_version,
            "user_hash": self.user_hash,
            "device_fingerprint": self.device_fingerprint,
            "install_token": install_token,
            "timestamp": timestamp or int(time.time()),
        }
        return _post_json(f"{self.api_base}/data", payload)

    def send_event(self, install_token: str, event: str, count: int = 1, timestamp=None) -> dict:
        payload = {
            "plugin_id": self.plugin_id,
            "version": self.plugin_version,
            "client_name": self.client_name,
            "client_version": self.client_version,
            "user_hash": self.user_hash,
            "device_fingerprint": self.device_fingerprint,
            "install_token": install_token,
            "event": event,
            "count": count,
            "timestamp": timestamp or int(time.time()),
        }
        return _post_json(f"{self.api_base}/event", payload)

_PROXY_SCHEMES = ("vless://", "vmess://", "trojan://", "ss://")
VLESS_HEALTH_INTERVAL_SEC = 15.0
VLESS_HEALTH_FAIL_THRESHOLD = 2

PROXY_API_URLS = {
    0: ["https://exitfy.ishrimp.xyz/proxy/latest"],
    1: ["https://exitfy.vpnetproxy.xyz/proxy/latest"],
    2: ["https://a.stemstep.cloud/proxy/socks"],
}
PROVIDER_NAMES = {0: "Shrimp", 1: "VPNET", 2: "Sworkle"}
VLESS_PROVIDER_SUBS = { 
    0: ["https://subs.invisibleshrimp.su/T1JSS-y0uNkBdCy-"],
    1: ["https://sub.vpnet.club/7q6wzBFL8huaB1nU"],
    2: ["https://s3.stemstep.cloud/sub/exitfy"],
}
SPONSOR_VLESS_URLS = set([u for arr in VLESS_PROVIDER_SUBS.values() for u in arr])
REFERRAL = {
    0: "https://t.me/invisibleshrimpbot?start=exitfy",
    1: "https://t.me/vpnetvpn_bot?start=exitfy",
    2: "https://t.me/sworklevpnbot?start=ref1208",
}
CHECK_INTERVAL_SEC = 5
RANDOM_PROXY_CHANCE = 0.15

TGWS_KIND = "tgws"
TGWS_LOCAL_PORT = 10805
TGWS_LOCAL_READY_TIMEOUT_SEC = float(os.getenv("TGWS_LOCAL_READY_TIMEOUT_SEC", "6"))
TGWS_WS_CONNECT_TIMEOUT_SEC = float(os.getenv("TGWS_WS_CONNECT_TIMEOUT_SEC", "4"))

VLESS_LOCAL_HOST = "127.0.0.1"
vless_local_host = VLESS_LOCAL_HOST
TGWS_WS_FAIL_COOLDOWN_SEC = 60.0

TGWS_DEFAULT_DC_IP = {
    "2": "149.154.167.220",
    "4": "149.154.167.220",
}

TRANSLATIONS = {
    "settings_header":      ("Настройки плагина", "Plugin settings"),
    "enable_plugin":        ("Включить плагин", "Enable plugin"),
    "open_tg_settings":     ("Настройки Telegram", "Telegram settings"),
    "select_mode":          ("Режим подключения", "Connection mode"),
    "mode_proxy":           ("Прокси", "Proxy"),
    "mode_vless":           ("VLESS", "VLESS"),
    "mode_tgws":            ("TG WS-прокси", "TG WS proxy"),
    "menu_settings":        ("Настройки exitFy", "exitFy settings"),
    "select_provider":      ("Провайдер", "Provider"),
    "node":                 ("Сервер", "Server"),
    "refresh":              ("Обновить список", "Refresh list"),
    "auto_switch":          ("Автопереключение", "Auto switch"),
    "auto_switch_desc":     ("Автоматически менять сервер при потере соединения", "Automatically switch server on connection loss"),
    "auto_switch_header":   ("Параметры автопереключения", "Auto switch parameters"),
    "connect_try":          ("Ожидание подключения (сек)", "Connect wait (sec)"),
    "stuck":                ("Порог зависания (сек)", "Stuck threshold (sec)"),
    "cooldown":             ("Кулдаун переключения (сек)", "Switch cooldown (sec)"),
    "referral_btn":         ("Включить интернет везде", "Enable internet everywhere"),
    "vless_provider":       ("Провайдер", "Provider"),
    "vless_prov_custom":    ("Кастом", "Custom"),
    "vless_nodes_header":   ("Узлы подключения", "Connection nodes"),
    "vless_add_key":        ("Ключ VLESS", "VLESS key"),
    "vless_add_sub":        ("Подписка VLESS", "VLESS subscription"),
    "vless_no_nodes":       ("Смените провайдера или добавьте свой", "Change provider or add custom"),
    "vless_nodes_manage":   ("Управление узлами", "Manage nodes"),
    "vless_nodes_update":   ("Обновить узлы", "Update nodes"),
    "vless_nodes_clear":    ("Очистить свои узлы", "Clear custom nodes"),
    "vless_node_default":   ("VLESS узел", "VLESS node"),
    "addon_download_title": ("VLESS", "VLESS"),
    "addon_downloading":    ("Скачивание дополнения...", "Downloading add-on..."),
    "addon_download_error": ("Ошибка скачивания", "Download error"),
    "addon_delete":         ("Удалить дополнение", "Delete add-on"),
    "addon_delete_title":   ("Удалить дополнение?", "Delete add-on?"),
    "addon_delete_msg":     ("Дополнение будет удалено.", "The add-on will be removed."),
    "btn_delete":           ("Удалить", "Delete"),
    "btn_cancel":           ("Отмена", "Cancel"),
    "addon_delete_error":   ("Ошибка удаления", "Delete error"),
    "vless_no_core_msg":    ("Нет ядра VLESS.\nНажмите кнопку ниже, чтобы скачать дополнение.", "VLESS core is not available.\nTap the button below to download the add-on."),
    "vless_download_addon": ("Скачать дополнение", "Download add-on"),
    "settings_error":       ("Ошибка настроек", "Settings error"),
    "proxy_settings_error": ("Ошибка настроек прокси", "Proxy settings error"),
    "proxy_node":           ("Узел", "Node"),
    "vless_custom_sub_err": ("Ошибка подписки", "Subscription error"),
    "vless_custom_sub_empty": ("Подписка не содержит узлов", "Subscription contains no nodes"),
    "vless_custom_sub_bad": ("Неверная ссылка подписки", "Invalid subscription URL"),
    "vless_app_not_supported": ("Приложение не поддерживается", "App is not supported"),
}

def Z(key):
    lang = Locale.getDefault().getLanguage()
    idx = 0 if str(lang).startswith("ru") else 1
    try: return str(TRANSLATIONS.get(key, (key, key))[idx])
    except: return str(key)

def _prefs():
    return ApplicationLoader.applicationContext.getSharedPreferences("exitfy_prefs", 0)

def _pget_int(k, d=0):
    try: return int(_prefs().getInt(k, int(d)))
    except: return d

def _pset_int(k, v):
    try: ed = _prefs().edit(); ed.putInt(k, int(v)); ed.commit()
    except: pass

def _pget_str(k, d=""):
    try: return str(_prefs().getString(k, str(d)) or "")
    except: return d

def _pset_str(k, v):
    try: ed = _prefs().edit(); ed.putString(k, str(v or "")); ed.commit()
    except: pass

def _to_int(v, d=0):
    try: return int(v)
    except: return int(d)

def _norm(s):
    try: s = str(s or "")
    except: return ""
    try: s = unescape(s)
    except: pass
    s = s.strip()
    if not s: return ""
    try: s = re.sub(r"[\s\)\]>,\.;]+$", "", s)
    except: pass
    try: s = re.sub(r"^[\s\(\[<]+", "", s)
    except: pass
    return s

def _is_emoji_char(ch):
    try:
        cp = ord(ch)
    except:
        return False
    if cp in (0x200D, 0xFE0E, 0xFE0F):
        return True
    if 0x1F1E6 <= cp <= 0x1F1FF:
        return True
    if 0x1F300 <= cp <= 0x1FAFF:
        return True
    if 0x2600 <= cp <= 0x27BF:
        return True
    return False

def _clean_vless_name(raw_name):
    try:
        s = str(raw_name or "")
    except:
        s = ""
    try:
        s = urllib.parse.unquote(s)
    except:
        pass
    out = []
    for ch in s:
        out.append(ch)
    s = "".join(out).strip()
    try:
        s = re.sub(r"\s{2,}", " ", s)
    except:
        pass
    if not s:
        s = Z("vless_node_default")
    return s

def _is_proxy_uri(s):
    try:
        return any(str(s).startswith(scheme) for scheme in _PROXY_SCHEMES)
    except:
        return False

def _find_proxy_scheme(s):
    for scheme in _PROXY_SCHEMES:
        idx = str(s).find(scheme)
        if idx >= 0:
            return scheme, idx
    return None, -1

def _sanitize_vless_uri(raw_uri):
    try:
        s = str(raw_uri or "")
    except:
        return ""
    try:
        s = unescape(s)
    except:
        pass
    s = s.replace("\n", "").replace("\r", "").strip()
    if not s:
        return ""

    try:
        scheme, idx = _find_proxy_scheme(s)
        if scheme and idx > 0:
            s = s[idx:]
    except:
        pass

    try:
        s = re.sub(r"[\s\)\]>,\.;]+$", "", s)
    except:
        pass

    if not _is_proxy_uri(s):
        return s
    if s.startswith("vmess://"):
        return s
    if "#" not in s:
        return s
    try:
        base, frag = s.split("#", 1)
        frag = urllib.parse.quote(urllib.parse.unquote(frag), safe="")
        return base + "#" + frag
    except:
        return s

def _uri_dedup_key(raw_uri):
    try:
        u = _sanitize_vless_uri(raw_uri)
        if not u:
            return ""
        if "#" in u:
            u = u.split("#", 1)[0]
        u = u.strip()
        if not u:
            return ""
        if u.startswith("vmess://"):
            return u
        try:
            p = urllib.parse.urlsplit(u)
        except Exception:
            p = None
        if not p or not p.scheme:
            return u
        scheme = str(p.scheme or "").lower()
        netloc = str(p.netloc or "")
        path = str(p.path or "")
        query = str(p.query or "")
        if netloc and "@" in netloc:
            userinfo, hostport = netloc.rsplit("@", 1)
        else:
            userinfo, hostport = "", netloc
        host = hostport
        port = ""
        if hostport.startswith("["):
            try:
                rb = hostport.find("]")
                if rb > 0:
                    host = hostport[: rb + 1]
                    rest = hostport[rb + 1 :]
                    if rest.startswith(":"):
                        port = rest[1:]
            except Exception:
                host = hostport
        elif ":" in hostport:
            try:
                host, port = hostport.rsplit(":", 1)
            except Exception:
                host, port = hostport, ""
        host = str(host or "").lower().strip()
        port = str(port or "").strip()
        if userinfo:
            userinfo = str(userinfo).strip()
            try:
                userinfo = urllib.parse.quote(urllib.parse.unquote(userinfo), safe=":")
            except Exception:
                pass
        if path:
            path = str(path).rstrip("/")
        q_items = []
        if query:
            try:
                q_items = urllib.parse.parse_qsl(query, keep_blank_values=True)
            except Exception:
                q_items = []
        if q_items:
            try:
                q_items.sort(key=lambda kv: (str(kv[0]), str(kv[1])))
            except Exception:
                pass
            try:
                query = urllib.parse.urlencode(q_items, doseq=True)
            except Exception:
                pass
        netloc2 = host
        if port:
            netloc2 = f"{host}:{port}"
        if userinfo:
            netloc2 = f"{userinfo}@{netloc2}"
        return urllib.parse.urlunsplit((scheme, netloc2, path, query, ""))
    except:
        return ""

def _parse_link(link):
    try:
        link = _norm(str(link))
        if not link: return None
        if link.startswith("https://t.me/socks"):
            link = link.replace("https://t.me/socks", "tg://socks", 1)
        elif link.startswith("https://t.me/proxy"):
            link = link.replace("https://t.me/proxy", "tg://proxy", 1)
        if "?" not in link: return None
        qs = link.split("?", 1)[1]
        params = {}
        for part in qs.split("&"):
            if "=" in part:
                k, v = part.split("=", 1); params[k.lower()] = urllib.parse.unquote(v)
        server = params.get("server", ""); port_s = params.get("port", "")
        if not server or not port_s: return None
        return SharedConfig.ProxyInfo(str(server), int(port_s), "", "", str(params.get("secret", "")))
    except: return None

def _is_bypass_host(p):
    try:
        h = str(p.address or "").lower()
        return ("ishrimp.xyz" in h or "vpnet" in h or "sworkle" in h or
                "stemstep" in h or h == "127.0.0.1" or h.startswith("exitfy-"))
    except: return False

def _apply_proxy_info(info):
    if info is None: return
    try: SharedConfig.loadProxyList()
    except: pass
    existing = None
    try:
        if hasattr(SharedConfig, "proxyList") and SharedConfig.proxyList is not None:
            for i in range(int(SharedConfig.proxyList.size())):
                try:
                    p = SharedConfig.proxyList.get(i)
                    if _is_bypass_host(p): existing = p; break
                except: continue
    except: pass
    added = None
    if existing is not None:
        try:
            existing.address = str(info.address); existing.port = int(info.port)
            existing.username = ""; existing.password = ""
            existing.secret = str(info.secret or ""); added = existing
        except: pass
    if added is None:
        try:
            added = SharedConfig.addProxy(str(info.address), int(info.port), "", "", str(info.secret or ""))
        except: added = info
    try: SharedConfig.currentProxy = added
    except: pass
    enabled_now = _tg_proxy_enabled()
    try: SharedConfig.proxyEnabled = bool(enabled_now); SharedConfig.saveConfig()
    except: pass
    try:
        ed = MessagesController.getGlobalMainSettings().edit()
        ed.putString("proxy_ip", str(added.address)); ed.putInt("proxy_port", int(added.port))
        ed.putString("proxy_user", ""); ed.putString("proxy_pass", "")
        ed.putString("proxy_secret", str(added.secret or ""))
        ed.putBoolean("proxy_enabled", bool(enabled_now)); ed.apply()
    except: pass
    try:
        if bool(enabled_now):
            ConnectionsManager.setProxySettings(True, added.address, int(added.port), "", "", added.secret)
        else:
            ConnectionsManager.setProxySettings(False, "", 0, "", "", "")
    except: pass
    try:
        run_on_ui_thread(lambda: NotificationCenter.getGlobalInstance().postNotificationName(
            NotificationCenter.proxySettingsChanged), 350)
    except: pass

def _tg_proxy_enabled():
    try: return bool(MessagesController.getGlobalMainSettings().getBoolean("proxy_enabled", False))
    except: return False

def _set_tg_proxy_enabled(enabled):
    try:
        ed = MessagesController.getGlobalMainSettings().edit()
        ed.putBoolean("proxy_enabled", bool(enabled)); ed.commit()
    except: pass
    try: SharedConfig.proxyEnabled = bool(enabled); SharedConfig.saveConfig()
    except: pass
    try:
        for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
            try:
                cm = ConnectionsManager.getInstance(ac)
                if cm:
                    if bool(enabled) and SharedConfig.currentProxy is not None:
                        p = SharedConfig.currentProxy
                        cm.setProxySettings(True, p.address, int(p.port), p.username, p.password, p.secret)
                    else:
                        cm.setProxySettings(False, "", 0, "", "", "")
            except: pass
    except: pass
    try: NotificationCenter.getGlobalInstance().postNotificationName(NotificationCenter.proxySettingsChanged)
    except: pass

def _fetch_proxy_links(proxy_type):
    urls = PROXY_API_URLS.get(proxy_type) or []
    out = []
    for url in urls:
        try:
            r = requests.get(url, timeout=10)
            if r.status_code != 200: continue
            text = (r.text or "").strip()
            if not text: continue
            for line in str(text).splitlines():
                s = (line or "").strip()
                if not s: continue
                found = []
                try: found = re.findall(r"(tg://[^\s\"']+)", s)
                except: pass
                if found:
                    out.extend([_norm(x) for x in found if _norm(x)]); continue
                try: found = re.findall(r"(https?://t\.me/[^\s\"']+)", s)
                except: pass
                if found:
                    out.extend([_norm(x) for x in found if _norm(x)]); continue
                if "tg://" in s or "t.me/" in s:
                    ns = _norm(s)
                    if ns: out.append(ns)
            if out: return out
        except: continue
    return out

def _proxy_latency(link):
    try:
        info = _parse_link(str(link))
        if not info: return None
        start = time.monotonic()
        s = socket.create_connection((str(info.address), int(info.port)), timeout=3)
        dt = float(time.monotonic() - start); s.close(); return dt
    except: return None

def _score_links(links):
    results = []; lock = threading.Lock()
    def worker(l):
        dt = _proxy_latency(str(l))
        if dt is not None:
            with lock: results.append((float(dt), str(l)))
    threads = [threading.Thread(target=worker, args=(l,), daemon=True) for l in links[:20]]
    for t in threads: t.start()
    deadline = time.time() + 5
    for t in threads:
        remain = float(deadline - time.time())
        if remain <= 0: break
        t.join(timeout=remain)
    results.sort(key=lambda x: x[0])
    return results if results else [(0.0, str(l)) for l in links[:5]]

def _log(msg): log(f"[exitfy] {msg}")

def _dlog(msg):
    if EXITFY_VERBOSE:
        _log(msg)

class VlessConfigParser:
    @staticmethod
    def _build_stream(params):
        net = params.get("type", "tcp")
        security = params.get("security", "none")
        fp = params.get("fp", "chrome")
        stream = {"network": net, "security": security}

        if security == "reality":
            stream["realitySettings"] = {
                "show": False, "fingerprint": fp,
                "serverName": params.get("sni", ""),
                "publicKey": params.get("pbk", ""),
                "shortId": params.get("sid", ""),
                "spiderX": params.get("spx", ""),
            }
        elif security == "tls":
            tls = {"serverName": params.get("sni", ""), "allowInsecure": False, "fingerprint": fp}
            alpn = params.get("alpn", "")
            if alpn:
                try:
                    tls["alpn"] = str(alpn).split(",")
                except:
                    pass
            stream["tlsSettings"] = tls

        try:
            if "packetEncoding" not in stream:
                stream["packetEncoding"] = str(params.get("packetEncoding", "xudp"))
        except:
            pass

        if net == "ws":
            stream["wsSettings"] = {"path": params.get("path", "/"),
                                     "headers": {"Host": params.get("host", params.get("sni", ""))}}
        elif net == "grpc":
            stream["grpcSettings"] = {"serviceName": params.get("serviceName", ""),
                                       "multiMode": params.get("mode", "multi") == "multi"}
        elif net in ("h2", "http"):
            host = params.get("host", params.get("sni", ""))
            stream["httpSettings"] = {"path": params.get("path", "/"),
                                       "host": [host] if host else []}
        elif net in ("kcp", "mkcp"):
            stream["kcpSettings"] = {"header": {"type": params.get("headerType", "none")},
                                      "seed": params.get("seed", "")}
        elif net in ("xhttp", "splithttp"):
            stream["xhttpSettings"] = {"path": params.get("path", "/"),
                                        "host": params.get("host", params.get("sni", ""))}
        elif net == "httpupgrade":
            stream["httpupgradeSettings"] = {"path": params.get("path", "/"),
                                              "host": params.get("host", params.get("sni", ""))}
        return stream

    @staticmethod
    def _base_config(local_port):
        return {
            "log": {"loglevel": "none"},
            "inbounds": [{"tag": "socks-in", "port": local_port, "listen": "127.0.0.1",
                          "protocol": "socks",
                          "settings": {"auth": "noauth", "udp": True, "ip": "127.0.0.1"},
                          "sniffing": {"enabled": True, "destOverride": ["http", "tls"], "routeOnly": False}}],
            "outbounds": [{"tag": "proxy"}],
        }

    @staticmethod
    def _parse_std_uri(uri_str):
        name = Z("vless_node_default")
        if "#" in uri_str:
            uri_str, frag = uri_str.split("#", 1)
            name = _clean_vless_name(frag)
            if not name:
                name = Z("vless_node_default")
        scheme_end = uri_str.find("://")
        if scheme_end < 0:
            return None

        body = uri_str[scheme_end + 3:]
        if "@" not in body:
            return None
        user_info, rest = body.split("@", 1)
        if "?" in rest:
            host_port, params_raw = rest.split("?", 1)
        else:
            host_port, params_raw = rest, ""
        host_port = str(host_port).strip().rstrip("/")
        if ":" not in host_port:
            return None
        host, port_str = host_port.split(":", 1)
        try:
            port = int(str(port_str).strip())
        except:
            return None
        params = {}
        if params_raw:
            for pair in params_raw.split("&"):
                if "=" in pair:
                    k, v = pair.split("=", 1)
                    params[k] = urllib.parse.unquote(v)
        return {"user": user_info, "host": host, "port": port, "params": params, "name": name}

    @staticmethod
    def parse(uri_str, local_port):
        try:
            uri_str = _sanitize_vless_uri(uri_str)
            if uri_str.startswith("vless://"):
                return VlessConfigParser._parse_vless(uri_str, local_port)
            if uri_str.startswith("vmess://"):
                return VlessConfigParser._parse_vmess(uri_str, local_port)
            if uri_str.startswith("trojan://"):
                return VlessConfigParser._parse_trojan(uri_str, local_port)
            if uri_str.startswith("ss://"):
                return VlessConfigParser._parse_ss(uri_str, local_port)
            return None, None
        except Exception as e:
            _log(f"config parse error: {e}"); return None, None

    @staticmethod
    def _parse_vless(uri_str, local_port):
        r = VlessConfigParser._parse_std_uri(uri_str)
        if not r:
            return None, None
        uuid_str = r["user"]
        params = r["params"]
        flow = params.get("flow", "")
        users = [{"id": uuid_str, "encryption": "none"}]
        if flow:
            users[0]["flow"] = flow
        config = VlessConfigParser._base_config(local_port)
        config["outbounds"][0].update({
            "protocol": "vless",
            "settings": {"vnext": [{"address": r["host"], "port": r["port"], "users": users}]},
            "streamSettings": VlessConfigParser._build_stream(params),
        })
        return json.dumps(config), r["name"]

    @staticmethod
    def _parse_vmess(uri_str, local_port):
        try:
            b64_part = uri_str[8:]
            if "#" in b64_part:
                b64_part = b64_part.split("#", 1)[0]
            b64_part = b64_part.strip().replace("-", "+").replace("_", "/")
            missing = len(b64_part) % 4
            if missing:
                b64_part += "=" * (4 - missing)
            raw = base64.b64decode(b64_part, validate=False)
            obj = json.loads(raw.decode("utf-8", errors="ignore"))
        except Exception:
            return None, None

        name = _clean_vless_name(obj.get("ps", "")) or Z("vless_node_default")
        host = obj.get("add", "")
        try:
            port = int(obj.get("port", 0))
        except:
            return None, None
        if not host or not port:
            return None, None

        uuid_str = obj.get("id", "")
        aid = _to_int(obj.get("aid", 0), 0)
        scy = obj.get("scy", "auto") or "auto"
        users = [{"id": uuid_str, "alterId": aid, "security": scy}]

        net = obj.get("net", "tcp") or "tcp"
        tls_val = obj.get("tls", "") or ""
        sni = obj.get("sni", "") or ""
        fp_val = obj.get("fp", "chrome") or "chrome"
        alpn_val = obj.get("alpn", "") or ""
        path_val = obj.get("path", "/") or "/"
        host_val = obj.get("host", "") or ""
        header_type = obj.get("type", "none") or "none"

        params = {
            "type": net,
            "security": "tls" if tls_val == "tls" else "none",
            "sni": sni or host_val,
            "fp": fp_val,
            "alpn": alpn_val,
            "path": path_val,
            "host": host_val or sni,
            "headerType": header_type,
            "serviceName": obj.get("path", ""),
            "seed": obj.get("path", ""),
        }

        config = VlessConfigParser._base_config(local_port)
        config["outbounds"][0].update({
            "protocol": "vmess",
            "settings": {"vnext": [{"address": host, "port": port, "users": users}]},
            "streamSettings": VlessConfigParser._build_stream(params),
        })
        return json.dumps(config), name

    @staticmethod
    def _parse_trojan(uri_str, local_port):
        r = VlessConfigParser._parse_std_uri(uri_str)
        if not r:
            return None, None
        password = urllib.parse.unquote(r["user"])
        params = r["params"]
        if not params.get("security"):
            params["security"] = "tls"
        if not params.get("sni"):
            params["sni"] = r["host"]

        config = VlessConfigParser._base_config(local_port)
        config["outbounds"][0].update({
            "protocol": "trojan",
            "settings": {"servers": [{"address": r["host"], "port": r["port"], "password": password}]},
            "streamSettings": VlessConfigParser._build_stream(params),
        })
        return json.dumps(config), r["name"]

    @staticmethod
    def _parse_ss(uri_str, local_port):
        try:
            body = uri_str[5:]
            name = Z("vless_node_default")
            if "#" in body:
                body, frag = body.split("#", 1)
                name = _clean_vless_name(frag) or Z("vless_node_default")
            body = body.strip()

            method = ""
            password = ""
            host = ""
            port = 0

            if "@" in body:
                user_part, server_part = body.split("@", 1)
                server_part = server_part.split("?", 1)[0].strip().rstrip("/")
                try:
                    user_part = user_part.replace("-", "+").replace("_", "/")
                    miss = len(user_part) % 4
                    if miss:
                        user_part += "=" * (4 - miss)
                    decoded = base64.b64decode(user_part, validate=False).decode("utf-8", errors="ignore")
                    if ":" in decoded:
                        method, password = decoded.split(":", 1)
                    else:
                        method = decoded
                except:
                    if ":" in user_part:
                        method, password = user_part.split(":", 1)
                    else:
                        return None, None
                if ":" not in server_part:
                    return None, None
                host, port_str = server_part.rsplit(":", 1)
                port = int(port_str)
            else:
                try:
                    b64 = body.replace("-", "+").replace("_", "/")
                    miss = len(b64) % 4
                    if miss:
                        b64 += "=" * (4 - miss)
                    decoded = base64.b64decode(b64, validate=False).decode("utf-8", errors="ignore")
                    if "@" not in decoded or ":" not in decoded:
                        return None, None
                    user_part, server_part = decoded.split("@", 1)
                    if ":" in user_part:
                        method, password = user_part.split(":", 1)
                    if ":" in server_part:
                        host, port_str = server_part.rsplit(":", 1)
                        port = int(port_str)
                except:
                    return None, None

            if not method or not host or not port:
                return None, None

            config = VlessConfigParser._base_config(local_port)
            config["outbounds"][0].update({
                "protocol": "shadowsocks",
                "settings": {"servers": [{"address": host, "port": port,
                                          "method": method, "password": password}]},
                "streamSettings": {"network": "tcp", "security": "none"},
            })
            return json.dumps(config), name
        except Exception:
            return None, None

class VlessCore:
    def __init__(self):
        self.lib = None
        self.mode = None
        self.libc = None

        self._try_init_standalone()

    def _try_init_standalone(self):
        try:
            ctx = ApplicationLoader.applicationContext
            dir_path = os.path.join(str(ctx.getFilesDir()), "exitfy_modules")
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)
            so_path = os.path.join(dir_path, "libvless.so")

            if (not os.path.exists(so_path)) or (os.path.getsize(so_path) < 1024):
                return

            linker = _NativeLinker()
            lib = linker.load(so_path)
            if not lib:
                return

            lib.StartCore.argtypes = [ctypes.c_char_p]
            lib.StartCore.restype = ctypes.c_void_p
            lib.StopCore.argtypes = []
            lib.StopCore.restype = None
            self.lib = lib
            self.mode = "standalone"

            try:
                self.libc = ctypes.CDLL("libc.so")
                self.libc.free.argtypes = [ctypes.c_void_p]
                self.libc.free.restype = None
            except:
                self.libc = None
        except Exception as e:
            _log(f"libvless standalone init failed: {e}")
            return

    def is_available(self): return self.lib is not None

    def start(self, config_json):
        if not self.is_available(): return "Native library not loaded"
        try:
            res_ptr = self.lib.StartCore(config_json.encode('utf-8'))
            if not res_ptr:
                return ""
            try:
                res_str = ctypes.cast(res_ptr, ctypes.c_char_p).value
                out = res_str.decode('utf-8', errors='ignore') if res_str else ""
            except:
                out = ""
            try:
                if self.libc:
                    self.libc.free(res_ptr)
            except:
                pass
            return out
        except Exception as e: return str(e)

    def stop(self):
        if self.is_available():
            try: self.lib.StopCore()
            except: pass

class _NativeLinker:
    def __init__(self):
        self.libdl = ctypes.CDLL("libdl.so")
        try:
            self.open_ext = self.libdl.android_dlopen_ext
            class Info(ctypes.Structure):
                _fields_ = [
                    ("flags", ctypes.c_uint64),
                    ("reserved_addr", ctypes.c_void_p),
                    ("reserved_size", ctypes.c_size_t),
                    ("relro_fd", ctypes.c_int),
                    ("library_fd", ctypes.c_int),
                    ("library_fd_offset", ctypes.c_uint64),
                    ("library_namespace", ctypes.c_void_p),
                ]
            self.Info = Info
            self.open_ext.argtypes = [ctypes.c_char_p, ctypes.c_int, ctypes.POINTER(self.Info)]
            self.open_ext.restype = ctypes.c_void_p
        except:
            self.open_ext = None

    def load(self, path: str):
        if not os.path.exists(path):
            return None

        try:
            if int(Build.VERSION.SDK_INT) >= 29 and self.open_ext:
                fd = os.open(path, os.O_RDONLY)
                try:
                    info = self.Info(flags=0x10, library_fd=fd)
                    h = self.open_ext(path.encode(), 2 | 0x100, ctypes.byref(info))
                    if h:
                        return ctypes.CDLL(path, handle=h)
                finally:
                    try: os.close(fd)
                    except: pass
        except:
            pass

        try:
            return ctypes.CDLL(path, mode=2 | 0x100)
        except:
            return None

def _vless_apply_tg_proxy(port, enable):
    try:
        ProxyInfoClass = find_class("org.telegram.messenger.SharedConfig$ProxyInfo")
        try:
            SharedConfig.loadProxyList()
        except:
            pass

        local_proxy = None
        try:
            if hasattr(SharedConfig, "proxyList") and SharedConfig.proxyList:
                duplicates = []
                for i in range(SharedConfig.proxyList.size()):
                    try:
                        p = SharedConfig.proxyList.get(i)
                        if str(p.address) == "127.0.0.1":
                            if local_proxy is None:
                                local_proxy = p
                            else:
                                duplicates.append(p)
                    except:
                        continue
                for p in duplicates:
                    try:
                        SharedConfig.deleteProxy(p)
                    except:
                        pass
        except:
            pass

        ed = MessagesController.getGlobalMainSettings().edit()
        try:
            proxy_list = SharedConfig.proxyList
            if proxy_list:
                to_remove = []
                for i in range(proxy_list.size()):
                    p = proxy_list.get(i)
                    if str(p.address) == "127.0.0.1":
                        to_remove.append(p)
                for p in to_remove:
                    SharedConfig.deleteProxy(p)
        except: pass

        try:
            curr = SharedConfig.currentProxy
            if curr and str(curr.address) == "127.0.0.1":
                SharedConfig.currentProxy = None
        except:
            pass

        if enable:
            if local_proxy is not None:
                try:
                    local_proxy.port = int(port)
                    local_proxy.username = ""
                    local_proxy.password = ""
                    local_proxy.secret = ""
                except:
                    pass
                proxy_obj = local_proxy
            else:
                proxy_obj = None
                try:
                    proxy_obj = SharedConfig.addProxy("127.0.0.1", int(port), "", "", "")
                except:
                    proxy_obj = None
                if not proxy_obj:
                    try:
                        proxy_obj = ProxyInfoClass("127.0.0.1", int(port), "", "", "")
                        try:
                            SharedConfig.addProxy(proxy_obj)
                        except:
                            pass
                    except:
                        proxy_obj = None

            try:
                if proxy_obj:
                    SharedConfig.currentProxy = proxy_obj
            except:
                pass

            try:
                SharedConfig.proxyEnabled = True
            except:
                pass

            ed.putString("proxy_ip", "127.0.0.1"); ed.putInt("proxy_port", port)
            try:
                ed.remove("proxy_user"); ed.remove("proxy_pass")
            except:
                ed.putString("proxy_user", ""); ed.putString("proxy_pass", "")
            ed.putString("proxy_secret", ""); ed.putBoolean("proxy_enabled", True)

            try:
                ed.putBoolean("proxy_enabled_calls", True)
            except:
                pass
            try:
                ed.putBoolean("proxy_calls_enabled", True)
            except:
                pass
            try:
                ed.putBoolean("calls_use_proxy", True)
            except:
                pass

            try:
                for attr in ("proxyEnabledCalls", "proxyCallsEnabled", "callsEnabled", "useProxyForCalls"):
                    try:
                        setattr(SharedConfig, attr, True)
                    except:
                        pass
            except:
                pass
            if proxy_obj:
                try:
                    for attr in ("enabledForCalls", "useForCalls"):
                        try:
                            setattr(proxy_obj, attr, True)
                        except:
                            pass
                except:
                    pass

            try:
                for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
                    try:
                        mc = MessagesController.getInstance(ac)
                        if mc:
                            for pname in ("mainconfig", "userconfing"):
                                try:
                                    prefs = mc.getSharedPreferences(pname, 0)
                                    if prefs:
                                        ed2 = prefs.edit()
                                        ed2.putBoolean("proxy_enabled_calls", True)
                                        ed2.putBoolean("proxy_calls_enabled", True)
                                        ed2.putBoolean("calls_use_proxy", True)
                                        ed2.commit()
                                except:
                                    pass
                    except:
                        pass
            except:
                pass

            try:
                for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
                    try:
                        prefs = MessagesController.getInstance(ac).getSharedPreferences("mainconfig", 0)
                        if prefs:
                            p2p_prev_key = "exitfy_p2p_backup_" + str(ac)
                            current_p2p = prefs.getInt("calls_p2p", -1)
                            if current_p2p != 2:
                                prefs.edit().putInt(p2p_prev_key, current_p2p).putInt("calls_p2p", 2).commit()
                    except:
                        pass
            except:
                pass
        else:
            try:
                curr = SharedConfig.currentProxy
                if curr and str(curr.address) == "127.0.0.1":
                    SharedConfig.currentProxy = None
            except:
                pass
            SharedConfig.proxyEnabled = False
            ed.putBoolean("proxy_enabled", False)

            try:
                for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
                    try:
                        prefs = MessagesController.getInstance(ac).getSharedPreferences("mainconfig", 0)
                        if prefs:
                            p2p_prev_key = "exitfy_p2p_backup_" + str(ac)
                            saved_p2p = prefs.getInt(p2p_prev_key, -1)
                            if saved_p2p >= 0:
                                prefs.edit().putInt("calls_p2p", saved_p2p).remove(p2p_prev_key).commit()
                    except:
                        pass
            except:
                pass
        try:
            ed.commit()
        except:
            ed.apply()
        try:
            SharedConfig.saveProxyList()
        except:
            pass
        SharedConfig.saveConfig()
        for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
            try:
                cm = ConnectionsManager.getInstance(ac)
                if cm:
                    if enable: cm.setProxySettings(True, "127.0.0.1", port, "", "", "")
                    else: cm.setProxySettings(False, "", 0, "", "", "")
            except: pass

        try:
            if enable:
                ConnectionsManager.setProxySettings(True, "127.0.0.1", int(port), "", "", "")
            else:
                ConnectionsManager.setProxySettings(False, "", 0, "", "", "")
        except:
            pass

        def _notify():
            try:
                try:
                    from java import jarray
                    from java.lang import Object
                    NotificationCenter.getGlobalInstance().postNotificationName(
                        NotificationCenter.proxySettingsChanged, jarray(Object)(0)
                    )
                except Exception:
                    NotificationCenter.getGlobalInstance().postNotificationName(
                        NotificationCenter.proxySettingsChanged
                    )
            except Exception:
                pass
        try:
            run_on_ui_thread(_notify, 300)
        except Exception:
            pass

        try:
            prefs = MessagesController.getGlobalMainSettings()
            _dlog(
                "vless_apply_tg_proxy: "
                f"enable={enable} port={port} "
                f"proxy_enabled={prefs.getBoolean('proxy_enabled', False)} "
                f"ip={str(prefs.getString('proxy_ip', '') or '')} "
                f"p={prefs.getInt('proxy_port', 0)} "
                f"secret_len={len(str(prefs.getString('proxy_secret', '') or ''))}"
            )
        except:
            _dlog(f"vless_apply_tg_proxy: enable={enable} port={port}")
    except Exception as e:
        _log(f"vless_apply_tg_proxy error: {e}")


def _check_vless_port(port, timeout=0.2):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(timeout)
            return s.connect_ex(('127.0.0.1', port)) == 0
    except:
        return False

def _get_free_port():
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(('', 0)); return s.getsockname()[1]


_TGWS_TG_RANGES = [
    (struct.unpack("!I", socket.inet_aton("185.76.151.0"))[0], struct.unpack("!I", socket.inet_aton("185.76.151.255"))[0]),
    (struct.unpack("!I", socket.inet_aton("149.154.160.0"))[0], struct.unpack("!I", socket.inet_aton("149.154.175.255"))[0]),
    (struct.unpack("!I", socket.inet_aton("91.105.192.0"))[0], struct.unpack("!I", socket.inet_aton("91.105.193.255"))[0]),
    (struct.unpack("!I", socket.inet_aton("91.108.0.0"))[0], struct.unpack("!I", socket.inet_aton("91.108.255.255"))[0]),
]

_TGWS_IP_TO_DC = {
    "149.154.175.50": (1, False),
    "149.154.175.51": (1, False),
    "149.154.175.53": (1, False),
    "149.154.175.54": (1, False),
    "149.154.175.52": (1, True),
    "149.154.167.41": (2, False),
    "149.154.167.50": (2, False),
    "149.154.167.51": (2, False),
    "149.154.167.220": (2, False),
    "95.161.76.100": (2, False),
    "149.154.167.151": (2, True),
    "149.154.167.222": (2, True),
    "149.154.167.223": (2, True),
    "149.154.175.100": (3, False),
    "149.154.175.101": (3, False),
    "149.154.175.102": (3, True),
    "149.154.167.91": (4, False),
    "149.154.167.92": (4, False),
    "149.154.164.250": (4, True),
    "149.154.166.120": (4, True),
    "149.154.166.121": (4, True),
    "149.154.167.118": (4, True),
    "149.154.165.111": (4, True),
    "91.108.56.100": (5, False),
    "91.108.56.101": (5, False),
    "91.108.56.116": (5, False),
    "91.108.56.126": (5, False),
    "149.154.171.5": (5, False),
    "91.108.56.102": (5, True),
    "91.108.56.128": (5, True),
    "91.108.56.151": (5, True),
    "91.105.192.100": (203, False),
}


def _tgws_unique_values(*values):
    out = []
    seen = set()
    for value in values:
        try:
            trimmed = str(value or "").strip()
        except:
            trimmed = ""
        if not trimmed or trimmed in seen:
            continue
        seen.add(trimmed)
        out.append(trimmed)
    return out


def _tgws_xor_mask(data, mask):
    if not mask:
        return data
    buf = bytearray(data)
    for idx in range(len(buf)):
        buf[idx] ^= mask[idx & 3]
    return bytes(buf)


def _tgws_build_frame(opcode, payload, mask=True):
    try:
        body = bytes(payload or b"")
    except:
        body = b""
    out = bytearray()
    out.append(0x80 | (int(opcode) & 0x0F))
    length = len(body)
    mask_bit = 0x80 if mask else 0x00
    if length < 126:
        out.append(mask_bit | length)
    elif length <= 0xFFFF:
        out.append(mask_bit | 126)
        out.extend(struct.pack(">H", length))
    else:
        out.append(mask_bit | 127)
        out.extend(struct.pack(">Q", length))
    if mask:
        mask_key = os.urandom(4)
        out.extend(mask_key)
        body = _tgws_xor_mask(body, mask_key)
    out.extend(body)
    return bytes(out)


def _tgws_read_exact(reader, size):
    buf = b""
    need = int(size or 0)
    while len(buf) < need:
        chunk = reader.read(need - len(buf))
        if not chunk:
            raise EOFError("unexpected eof")
        buf += chunk
    return buf


def _tgws_recv_exact(conn, size):
    data = b""
    need = int(size or 0)
    while len(data) < need:
        chunk = conn.recv(need - len(data))
        if not chunk:
            raise EOFError("unexpected eof")
        data += chunk
    return data


def _tgws_is_telegram_ip(ip):
    try:
        ip_s = str(ip or "")
    except Exception:
        ip_s = ""
    try:
        if ":" in ip_s:
            try:
                socket.inet_pton(socket.AF_INET6, ip_s)
                return True
            except Exception:
                pass
    except Exception:
        pass
    try:
        raw = struct.unpack("!I", socket.inet_aton(ip_s))[0]
        for lo, hi in _TGWS_TG_RANGES:
            if lo <= raw <= hi:
                return True
    except:
        pass
    return False


def _tgws_resolve_ipv4(hostname):
    try:
        hn = str(hostname or "").strip()
        if not hn:
            return None
        try:
            if ":" in hn:
                return None
            socket.inet_aton(hn)
            return hn
        except Exception:
            pass
        for family, _, _, _, sockaddr in socket.getaddrinfo(hn, None, socket.AF_INET, socket.SOCK_STREAM):
            if family == socket.AF_INET:
                return str(sockaddr[0])
    except Exception:
        return None
    return None


def _tgws_default_dc(dc_ip_map):
    try:
        m = dc_ip_map or {}
        if not isinstance(m, dict):
            return None
        for k in ("2", "1", "4", "3", "5"):
            if k in m and str(m.get(k) or "").strip():
                return int(k)
        for k, v in m.items():
            if str(v or "").strip():
                try:
                    return int(k)
                except Exception:
                    continue
    except Exception:
        pass
    return None


def _tgws_is_http_transport(data):
    try:
        raw = bytes(data or b"")
    except:
        raw = b""
    return raw[:5] == b"POST " or raw[:4] == b"GET " or raw[:5] == b"HEAD " or raw[:8] == b"OPTIONS "


def _tgws_keystream_java(key_bytes, iv_bytes, size):
    if JavaCipher is None or SecretKeySpec is None or IvParameterSpec is None:
        raise RuntimeError("Java crypto unavailable")
    cipher = JavaCipher.getInstance("AES/CTR/NoPadding")
    cipher.init(JavaCipher.ENCRYPT_MODE, SecretKeySpec(bytes(key_bytes), "AES"), IvParameterSpec(bytes(iv_bytes)))
    result = cipher.doFinal(b"\x00" * int(size))
    try:
        return bytes(result)
    except:
        return bytes(bytearray(result))


def _tgws_dc_from_init(data):
    try:
        raw = bytes(data or b"")
        if len(raw) < 64:
            return None, False
        key = raw[8:40]
        iv = raw[40:56]
        keystream = _tgws_keystream_java(key, iv, 64)
        plain = bytes(a ^ b for a, b in zip(raw[56:64], keystream[56:64]))
        proto = struct.unpack("<I", plain[0:4])[0]
        dc_raw = struct.unpack("<h", plain[4:6])[0]
        if proto in (0xEFEFEFEF, 0xEEEEEEEE, 0xDDDDDDDD):
            dc = abs(int(dc_raw))
            if 1 <= dc <= 1000:
                return dc, bool(dc_raw < 0)
    except:
        pass
    return None, False


def _tgws_patch_init_dc(data, dc_value):
    try:
        raw = bytes(data or b"")
    except Exception:
        return data
    if len(raw) < 64:
        return data
    try:
        key = raw[8:40]
        iv = raw[40:56]
        keystream = _tgws_keystream_java(key, iv, 64)
        replacement = struct.pack("<h", int(dc_value))
        patched = bytearray(raw[:64])
        patched[60] = keystream[60] ^ replacement[0]
        patched[61] = keystream[61] ^ replacement[1]
        if len(raw) > 64:
            return bytes(patched) + raw[64:]
        return bytes(patched)
    except Exception:
        return data


class _TgWsMsgSplitter:
    def __init__(self, init_data):
        raw = bytes(init_data or b"")
        key = raw[8:40]
        iv = raw[40:56]
        if JavaCipher is None or SecretKeySpec is None or IvParameterSpec is None:
            raise RuntimeError("Java crypto unavailable")
        cipher = JavaCipher.getInstance("AES/CTR/NoPadding")
        cipher.init(JavaCipher.ENCRYPT_MODE, SecretKeySpec(bytes(key), "AES"), IvParameterSpec(bytes(iv)))
        try:
            cipher.update(b"\x00" * 64)
        except Exception:
            pass
        self._cipher = cipher

    def split(self, chunk):
        try:
            plain = self._cipher.update(bytes(chunk))
            try:
                plain = bytes(plain)
            except Exception:
                plain = bytes(bytearray(plain))
        except Exception:
            return [chunk]

        boundaries = []
        pos = 0
        plen = len(plain)
        while pos < plen:
            first = plain[pos]
            if first == 0x7F:
                if pos + 4 > plen:
                    break
                msg_len = (struct.unpack_from("<I", plain, pos + 1)[0] & 0xFFFFFF) * 4
                pos += 4
            else:
                msg_len = first * 4
                pos += 1
            if msg_len == 0 or pos + msg_len > plen:
                break
            pos += msg_len
            boundaries.append(pos)

        if len(boundaries) <= 1:
            return [chunk]
        parts = []
        prev = 0
        for boundary in boundaries:
            parts.append(bytes(chunk)[prev:boundary])
            prev = boundary
        if prev < len(chunk):
            parts.append(bytes(chunk)[prev:])
        return parts


def _tgws_ws_domains(dc, is_media):
    try:
        dc_num = int(dc or 0)
    except:
        dc_num = 0
    bases = ["web.telegram.org", "telegram.org"]
    out = []
    for base in bases:
        if is_media:
            out.append(f"kws{dc_num}-1.{base}")
            out.append(f"kws{dc_num}.{base}")
        else:
            out.append(f"kws{dc_num}.{base}")
            out.append(f"kws{dc_num}-1.{base}")
    seen = set()
    uniq = []
    for d in out:
        if d not in seen:
            seen.add(d)
            uniq.append(d)
    return uniq


def _tgws_socks_reply(status):
    return bytes((0x05, int(status) & 0xFF, 0x00, 0x01, 0, 0, 0, 0, 0, 0))


def _tgws_read_socks_address(conn, atyp):
    atyp = int(atyp or 0)
    if atyp == 0x01:
        host = socket.inet_ntoa(_tgws_recv_exact(conn, 4))
        port = int.from_bytes(_tgws_recv_exact(conn, 2), "big")
        return host, port
    if atyp == 0x03:
        size = _tgws_recv_exact(conn, 1)[0]
        host = _tgws_recv_exact(conn, size).decode("utf-8", errors="ignore")
        port = int.from_bytes(_tgws_recv_exact(conn, 2), "big")
        return host, port
    if atyp == 0x04:
        host = socket.inet_ntop(socket.AF_INET6, _tgws_recv_exact(conn, 16))
        port = int.from_bytes(_tgws_recv_exact(conn, 2), "big")
        return host, port
    raise ValueError("unsupported atyp")


class _TgWsRawWebSocket:
    def __init__(self, sock, reader):
        self.sock = sock
        self.reader = reader
        self._lock = threading.Lock()
        self._closed = False

    @staticmethod
    def connect(connect_host, sni_host, timeout=TGWS_WS_CONNECT_TIMEOUT_SEC):
        sock = socket.create_connection((str(connect_host), 443), timeout=float(timeout))
        ctx = ssl.create_default_context()
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        wrapped = ctx.wrap_socket(sock, server_hostname=str(sni_host or ""))
        wrapped.settimeout(float(timeout))
        key = base64.b64encode(os.urandom(16)).decode("ascii")
        req = "\r\n".join([
            "GET /apiws HTTP/1.1",
            "Host: " + str(sni_host or ""),
            "Upgrade: websocket",
            "Connection: Upgrade",
            "Sec-WebSocket-Key: " + key,
            "Sec-WebSocket-Version: 13",
            "Sec-WebSocket-Protocol: binary",
            "Origin: https://web.telegram.org",
            "User-Agent: Mozilla/5.0 (Linux; Android 13)",
            "",
            "",
        ]).encode("utf-8")
        wrapped.sendall(req)
        reader = wrapped.makefile("rb")
        status_line = reader.readline().decode("utf-8", errors="replace").strip()
        parts = status_line.split(" ", 2)
        code = int(parts[1]) if len(parts) >= 2 and parts[1].isdigit() else 0
        while True:
            line = reader.readline().decode("utf-8", errors="replace")
            if not line or line in ("\r\n", "\n"):
                break
        wrapped.settimeout(None)
        if code != 101:
            try:
                reader.close()
            except:
                pass
            try:
                wrapped.close()
            except:
                pass
            raise RuntimeError(status_line or "bad websocket handshake")
        return _TgWsRawWebSocket(wrapped, reader)

    def send(self, payload):
        with self._lock:
            if self._closed:
                raise EOFError("websocket closed")
            self.sock.sendall(_tgws_build_frame(0x2, payload, mask=True))

    def send_batch(self, parts):
        with self._lock:
            if self._closed:
                raise EOFError("websocket closed")
            for payload in list(parts or []):
                self.sock.sendall(_tgws_build_frame(0x2, payload, mask=True))

    def _read_frame(self):
        header = _tgws_read_exact(self.reader, 2)
        opcode = header[0] & 0x0F
        masked = bool(header[1] & 0x80)
        length = header[1] & 0x7F
        if length == 126:
            length = struct.unpack(">H", _tgws_read_exact(self.reader, 2))[0]
        elif length == 127:
            length = struct.unpack(">Q", _tgws_read_exact(self.reader, 8))[0]
        mask_key = b""
        if masked:
            mask_key = _tgws_read_exact(self.reader, 4)
        payload = _tgws_read_exact(self.reader, length)
        if masked:
            payload = _tgws_xor_mask(payload, mask_key)
        return opcode, payload

    def recv(self):
        while True:
            opcode, payload = self._read_frame()
            if opcode == 0x8:
                self.close()
                return None
            if opcode == 0x9:
                self._write_control(0xA, payload)
                continue
            if opcode == 0xA:
                continue
            if opcode in (0x1, 0x2):
                return payload

    def _write_control(self, opcode, payload):
        with self._lock:
            if self._closed:
                return
            self.sock.sendall(_tgws_build_frame(opcode, payload, mask=True))

    def close(self):
        with self._lock:
            if self._closed:
                return
            self._closed = True
            try:
                self.sock.sendall(_tgws_build_frame(0x8, b"", mask=True))
            except:
                pass
            try:
                self.reader.close()
            except:
                pass
            try:
                self.sock.close()
            except:
                pass


class TgWsCore:
    def __init__(self):
        self.listener = None
        self.listener_thread = None
        self.stop_event = threading.Event()
        self.lock = threading.Lock()
        self.tracked = set()
        self.fail_until = {}
        self.ws_blacklist = set()
        self.ws_domain_pref = {}
        self._last_ws_event_at = 0.0
        self.port = 0
        self.dc_ip = dict(TGWS_DEFAULT_DC_IP)

    def is_available(self):
        return JavaCipher is not None

    def _track(self, closer):
        with self.lock:
            self.tracked.add(closer)

    def _untrack(self, closer):
        with self.lock:
            self.tracked.discard(closer)

    def _close_all_tracked(self):
        with self.lock:
            closers = list(self.tracked)
            self.tracked.clear()
        for closer in closers:
            try:
                closer.close()
            except:
                pass

    def _parse_config(self, config_json):
        cfg = {"port": int(TGWS_LOCAL_PORT), "dc_ip": dict(TGWS_DEFAULT_DC_IP)}
        raw = str(config_json or "").strip()
        if raw:
            parsed = json.loads(raw)
            if isinstance(parsed, dict):
                if parsed.get("port"):
                    cfg["port"] = int(parsed.get("port") or TGWS_LOCAL_PORT)
                dc_ip = parsed.get("dc_ip")
                if isinstance(dc_ip, dict):
                    for key, value in dc_ip.items():
                        cfg["dc_ip"][str(key)] = str(value)
        if cfg["port"] <= 0 or cfg["port"] > 65535:
            cfg["port"] = int(TGWS_LOCAL_PORT)
        return cfg

    def start(self, config_json):
        if not self.is_available():
            return "Java crypto недоступно"
        try:
            cfg = self._parse_config(config_json)
            self.stop()
            listener = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            listener.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            listener.bind((VLESS_LOCAL_HOST, int(cfg["port"])))
            listener.listen(32)
            listener.settimeout(1.0)
            self.listener = listener
            self.port = int(cfg["port"])
            self.dc_ip = dict(cfg.get("dc_ip") or {})
            self.stop_event.clear()
            self.listener_thread = threading.Thread(target=self._accept_loop, daemon=True)
            self.listener_thread.start()
            return ""
        except Exception as exc:
            try:
                if self.listener is not None:
                    self.listener.close()
            except:
                pass
            self.listener = None
            return str(exc)

    def stop(self):
        self.stop_event.set()
        try:
            if self.listener is not None:
                self.listener.close()
        except:
            pass
        self.listener = None
        self._close_all_tracked()
        thread = self.listener_thread
        self.listener_thread = None
        if thread is not None and thread.is_alive():
            try:
                thread.join(1.5)
            except:
                pass

    def _accept_loop(self):
        while not self.stop_event.is_set():
            try:
                conn, _ = self.listener.accept()
            except socket.timeout:
                continue
            except:
                if self.stop_event.is_set():
                    break
                continue
            self._track(conn)
            threading.Thread(target=self._handle_client, args=(conn,), daemon=True).start()

    def _close_write(self, conn):
        try:
            conn.shutdown(socket.SHUT_WR)
        except:
            pass

    def _bridge_tcp(self, left, right):
        done = threading.Event()

        def _forward(src, dst):
            try:
                while not done.is_set():
                    chunk = src.recv(65536)
                    if not chunk:
                        break
                    dst.sendall(chunk)
            except:
                pass
            done.set()
            self._close_write(dst)

        t1 = threading.Thread(target=_forward, args=(left, right), daemon=True)
        t2 = threading.Thread(target=_forward, args=(right, left), daemon=True)
        t1.start()
        t2.start()
        while not done.is_set() and (t1.is_alive() or t2.is_alive()):
            time.sleep(0.1)
        done.set()
        for item in (left, right):
            try:
                item.close()
            except:
                pass
            self._untrack(item)

    def _bridge_ws(self, client, ws):
        return self._bridge_ws_split(client, ws, None)

    def _bridge_ws_split(self, client, ws, splitter):
        done = threading.Event()

        def _client_to_ws():
            try:
                while not done.is_set():
                    chunk = client.recv(65536)
                    if not chunk:
                        break
                    if splitter is not None:
                        parts = splitter.split(chunk)
                        if len(parts) > 1:
                            try:
                                ws.send_batch(parts)
                            except Exception:
                                for p in parts:
                                    ws.send(p)
                        else:
                            ws.send(parts[0])
                    else:
                        ws.send(chunk)
            except:
                pass
            done.set()

        def _ws_to_client():
            try:
                while not done.is_set():
                    payload = ws.recv()
                    if not payload:
                        break
                    client.sendall(payload)
            except:
                pass
            done.set()

        t1 = threading.Thread(target=_client_to_ws, daemon=True)
        t2 = threading.Thread(target=_ws_to_client, daemon=True)
        t1.start()
        t2.start()
        while not done.is_set() and (t1.is_alive() or t2.is_alive()):
            time.sleep(0.1)
        done.set()
        try:
            ws.close()
        except:
            pass
        self._untrack(ws)
        try:
            client.close()
        except:
            pass
        self._untrack(client)

    def _direct_passthrough(self, client, dst, port):
        remote = socket.create_connection((str(dst), int(port)), 10.0)
        self._track(remote)
        client.sendall(_tgws_socks_reply(0x00))
        self._bridge_tcp(client, remote)

    def _tcp_fallback(self, client, dst, port, init_packet):
        remote = socket.create_connection((str(dst), int(port)), 10.0)
        self._track(remote)
        if init_packet:
            remote.sendall(bytes(init_packet))
        self._bridge_tcp(client, remote)

    def _handle_client(self, conn):
        try:
            conn.settimeout(10.0)
            header = _tgws_recv_exact(conn, 2)
            if header[0] != 0x05:
                return
            _tgws_recv_exact(conn, header[1])
            conn.sendall(b"\x05\x00")
            req = _tgws_recv_exact(conn, 4)
            if req[1] != 0x01:
                conn.sendall(_tgws_socks_reply(0x07))
                return
            atyp = int(req[3])
            dst, port = _tgws_read_socks_address(conn, atyp)

            dst_ip = str(dst)
            if atyp == 0x03:
                try:
                    resolved = _tgws_resolve_ipv4(dst)
                    if resolved:
                        dst_ip = resolved
                except Exception:
                    pass

            try:
                _dlog(f"TGWS socks connect atyp={atyp} dst={dst} dst_ip={dst_ip} port={int(port)}")
            except Exception:
                pass

            if not _tgws_is_telegram_ip(dst_ip):
                try:
                    try:
                        _dlog(f"TGWS passthrough non-TG dst_ip={dst_ip} dst={dst}")
                    except Exception:
                        pass
                    self._direct_passthrough(conn, dst, port)
                except:
                    try:
                        conn.sendall(_tgws_socks_reply(0x05))
                    except:
                        pass
                return
            conn.sendall(_tgws_socks_reply(0x00))
            conn.settimeout(15.0)
            init_packet = _tgws_recv_exact(conn, 64)
            conn.settimeout(None)
            if _tgws_is_http_transport(init_packet):
                return
            dc, is_media = _tgws_dc_from_init(init_packet)
            try:
                _dlog(f"TGWS init parsed dc={dc} is_media={is_media} dst_ip={dst_ip}")
            except Exception:
                pass

            try:
                if dc is not None and str(dc) not in (self.dc_ip or {}):
                    dc = None
            except Exception:
                pass

            init_patched = False
            if dc is None:
                try:
                    entry = _TGWS_IP_TO_DC.get(str(dst_ip))
                    if entry is not None:
                        dc = int(entry[0])
                        if not bool(is_media):
                            is_media = bool(entry[1])
                        try:
                            _dlog(f"TGWS dc inferred from ip map: dst_ip={dst_ip} dc={dc} is_media={is_media}")
                        except Exception:
                            pass
                        target_ip0 = str((self.dc_ip or {}).get(str(dc), "") or "").strip()
                        if target_ip0:
                            init_packet = _tgws_patch_init_dc(init_packet, dc if is_media else -dc)
                            init_patched = True
                except Exception:
                    pass

            if dc is None:
                try:
                    if _tgws_is_telegram_ip(dst_ip) and ":" in str(dst_ip):
                        dcd = _tgws_default_dc(self.dc_ip)
                        if dcd is not None:
                            dc = int(dcd)
                            is_media = False
                            try:
                                _dlog(f"TGWS dc defaulted for ipv6 dst_ip={dst_ip} dc={dc}")
                            except Exception:
                                pass
                except Exception:
                    pass

            if dc is None:
                try:
                    _dlog(f"TGWS fallback tcp (dc unknown) dst_ip={dst_ip}")
                except Exception:
                    pass
                self._tcp_fallback(conn, dst, port, init_packet)
                return
            target_ip = str((self.dc_ip or {}).get(str(dc), "") or "").strip()
            dc_key = (int(dc), bool(is_media))
            try:
                if dc_key in self.ws_blacklist:
                    self._tcp_fallback(conn, dst, port, init_packet)
                    return
            except Exception:
                pass
            fail_until = float(self.fail_until.get(dc_key, 0.0) or 0.0)
            if fail_until > time.time():
                try:
                    _dlog(f"TGWS cooldown fallback tcp dc={dc_key} until={fail_until}")
                except Exception:
                    pass
                self._tcp_fallback(conn, dst, port, init_packet)
                return
            ws = None
            last_ws_exc = None
            ws_failed_redirect = False
            all_redirects = True
            for candidate_ip in _tgws_unique_values(target_ip, dst_ip):
                domains = _tgws_ws_domains(dc, is_media)
                try:
                    pref = str(self.ws_domain_pref.get(dc_key) or "").strip()
                except Exception:
                    pref = ""
                if pref:
                    domains = [pref] + [d for d in domains if d != pref]

                for domain in domains:
                    if ws is None:
                        try:
                            ip4 = _tgws_resolve_ipv4(domain)
                            if ip4:
                                try:
                                    _dlog(f"TGWS ws dns resolved host={domain} ip4={ip4}")
                                except Exception:
                                    pass
                                ws = _TgWsRawWebSocket.connect(ip4, domain, timeout=TGWS_WS_CONNECT_TIMEOUT_SEC)
                            else:
                                ws = _TgWsRawWebSocket.connect(domain, domain, timeout=TGWS_WS_CONNECT_TIMEOUT_SEC)
                            try:
                                now = time.time()
                                if now - float(self._last_ws_event_at or 0.0) > 10.0:
                                    _log(f"TGWS websocket route active for dc={dc}{'m' if is_media else ''} host={domain}")
                                    self._last_ws_event_at = now
                                else:
                                    _dlog(f"TGWS ws ok (dns) dc={dc}{'m' if is_media else ''} host={domain}")
                            except Exception:
                                pass
                            try:
                                self.ws_domain_pref[dc_key] = str(domain)
                            except Exception:
                                pass
                            all_redirects = False
                            break
                        except Exception as e2:
                            last_ws_exc = e2
                            try:
                                _dlog(f"TGWS ws dns connect failed dc={dc}{'m' if is_media else ''} host={domain} err={e2}")
                            except Exception:
                                pass
                            try:
                                if str(e2).startswith("HTTP/") and " 30" in str(e2):
                                    ws_failed_redirect = True
                                    continue
                            except Exception:
                                pass
                            all_redirects = False
                            ws = None

                    if ws is None:
                        try:
                            if str(candidate_ip or "").strip():
                                ws = _TgWsRawWebSocket.connect(candidate_ip, domain, timeout=TGWS_WS_CONNECT_TIMEOUT_SEC)
                            if ws is not None:
                                try:
                                    now = time.time()
                                    if now - float(self._last_ws_event_at or 0.0) > 10.0:
                                        _log(f"TGWS websocket route active for dc={dc}{'m' if is_media else ''} host={domain}")
                                        self._last_ws_event_at = now
                                    else:
                                        _dlog(f"TGWS ws ok dc={dc}{'m' if is_media else ''} ip={candidate_ip} host={domain}")
                                except Exception:
                                    pass
                                try:
                                    self.ws_domain_pref[dc_key] = str(domain)
                                except Exception:
                                    pass
                                all_redirects = False
                                break
                        except Exception as e:
                            last_ws_exc = e
                            try:
                                _dlog(f"TGWS ws connect failed dc={dc}{'m' if is_media else ''} ip={candidate_ip} host={domain} err={e}")
                            except Exception:
                                pass
                            try:
                                if str(e).startswith("HTTP/") and " 30" in str(e):
                                    ws_failed_redirect = True
                                    continue
                            except Exception:
                                pass
                            all_redirects = False
                            ws = None
                if ws is not None:
                    break
            if ws is None:
                try:
                    if ws_failed_redirect and all_redirects:
                        self.ws_blacklist.add(dc_key)
                        _log(f"TGWS ws blacklist added for dc={dc_key}")
                except Exception:
                    pass
                try:
                    _dlog(f"TGWS ws failed -> tcp fallback dc={dc}{'m' if is_media else ''} dst_ip={dst_ip} last_err={last_ws_exc}")
                except Exception:
                    pass
                self.fail_until[dc_key] = time.time() + float(TGWS_WS_FAIL_COOLDOWN_SEC)
                self._tcp_fallback(conn, dst, port, init_packet)
                return
            self.fail_until.pop(dc_key, None)
            self._track(ws)
            ws.send(init_packet)
            splitter = None
            try:
                splitter = _TgWsMsgSplitter(init_packet)
            except Exception:
                splitter = None
            self._bridge_ws_split(conn, ws, splitter)
            return
        except:
            pass
        finally:
            try:
                conn.close()
            except:
                pass

def _fetch_vless_nodes(url):
    try:
        url2 = str(url or "")
        r = requests.get(url2, timeout=15)
        if r.status_code != 200: return []
        text = r.text.strip()
        if all(s not in text for s in _PROXY_SCHEMES):
            try:
                b64 = str(text or "").strip()
                b64 = b64.replace("\n", "").replace("\r", "")
                b64 = b64.replace("-", "+").replace("_", "/")
                missing = len(b64) % 4
                if missing:
                    b64 += '=' * (4 - missing)
                raw = base64.b64decode(b64, validate=False)
                text = raw.decode('utf-8', errors='ignore')
            except: pass
        nodes = []
        try:
            links = []
            for scheme in _PROXY_SCHEMES:
                try:
                    links.extend(re.findall(re.escape(scheme) + r"[^\s]+", str(text or "")))
                except:
                    continue
        except:
            links = []

        if not links:
            try:
                raw = str(text or "").lstrip()
                if raw.startswith("{") or raw.startswith("["):
                    def _walk(obj, out):
                        if isinstance(obj, dict):
                            t = obj.get("type")
                            p = obj.get("protocol")
                            if str(t).lower() == "vless" or str(p).lower() == "vless":
                                out.append(obj)
                            for v in obj.values():
                                _walk(v, out)
                        elif isinstance(obj, list):
                            for v in obj:
                                _walk(v, out)

                    try:
                        parsed = json.loads(raw)
                    except Exception as je:
                        parsed = None

                    if parsed is None:
                        raise Exception("json parse failed")

                    found = []
                    _walk(parsed, found)

                    built = []
                    for ob in found:
                        try:
                            uuid_str = ob.get("uuid") or ob.get("id") or ob.get("user")
                            server = ob.get("server") or ob.get("address")
                            port = ob.get("server_port") or ob.get("port")

                            if not uuid_str:
                                users = ob.get("users")
                                if isinstance(users, list) and users:
                                    u0 = users[0] if isinstance(users[0], dict) else None
                                    if u0:
                                        uuid_str = u0.get("uuid") or u0.get("id") or u0.get("user")

                            if (not uuid_str or not server or not port) and isinstance(ob.get("settings"), dict):
                                st = ob.get("settings") or {}
                                vnext = st.get("vnext")
                                if isinstance(vnext, list):
                                    for vn in vnext:
                                        try:
                                            if not isinstance(vn, dict):
                                                continue
                                            server2 = vn.get("address") or vn.get("server") or vn.get("address")
                                            port2 = vn.get("port")
                                            users2 = vn.get("users")
                                            uuid2 = None
                                            if isinstance(users2, list) and users2:
                                                u = users2[0] if isinstance(users2[0], dict) else None
                                                if u:
                                                    uuid2 = u.get("id") or u.get("uuid")
                                            if not uuid2 or not server2 or not port2:
                                                continue
                                            link2 = f"vless://{uuid2}@{server2}:{int(port2)}"
                                            tag2 = ob.get("tag") or ob.get("name") or ""
                                            if tag2:
                                                link2 += "#" + urllib.parse.quote(str(tag2))
                                            built.append(link2)
                                        except:
                                            continue
                                continue

                            if not uuid_str or not server or not port:
                                continue

                            tls = ob.get("tls") or {}
                            security = ""
                            sni = ""
                            if isinstance(tls, dict):
                                enabled = tls.get("enabled")
                                if enabled is True or str(enabled).lower() == "true":
                                    security = "tls"
                                sni = tls.get("server_name") or tls.get("sni") or ""

                            transport = ob.get("transport") or {}
                            net = "tcp"
                            path = ""
                            host = ""
                            if isinstance(transport, dict):
                                net = transport.get("type") or transport.get("network") or net
                                net = str(net or "tcp")
                                path = transport.get("path") or ""
                                headers2 = transport.get("headers") or {}
                                if isinstance(headers2, dict):
                                    host = headers2.get("Host") or headers2.get("host") or ""

                            params = {}
                            if security:
                                params["security"] = security
                            if sni:
                                params["sni"] = str(sni)
                            if net:
                                params["type"] = str(net)
                            if path:
                                params["path"] = str(path)
                            if host:
                                params["host"] = str(host)

                            qs = "&".join([f"{urllib.parse.quote(str(k))}={urllib.parse.quote(str(v))}" for k, v in params.items()])
                            tag = ob.get("tag") or ob.get("name") or ""
                            frag = urllib.parse.quote(str(tag)) if tag else ""
                            link = f"vless://{uuid_str}@{server}:{int(port)}"
                            if qs:
                                link += "?" + qs
                            if frag:
                                link += "#" + frag
                            built.append(link)
                        except:
                            continue

                    if built:
                        links.extend(built)
            except:
                pass

        if not links:
            for line in str(text or "").splitlines():
                line = str(line or "").strip()
                if line.startswith("vless://"):
                    links.append(line)

        filtered = 0
        any_not_supported = False
        seen_uris = set()
        for link in links:
            try:
                link = _sanitize_vless_uri(str(link or "").strip())
                if not _is_proxy_uri(link):
                    continue
                k = _uri_dedup_key(link)
                if not k:
                    continue
                if k in seen_uris:
                    continue
                seen_uris.add(k)
                try:
                    low = link.lower()
                    if "@0.0.0.0:1" in low:
                        filtered += 1
                        any_not_supported = True
                        continue
                    if "приложение" in urllib.parse.unquote(link).lower() and "не поддерж" in urllib.parse.unquote(link).lower():
                        filtered += 1
                        any_not_supported = True
                        continue
                except:
                    pass
                conf, name = VlessConfigParser.parse(link, 1080)
                if conf:
                    nodes.append({"name": name, "uri": link, "ping": -1})
            except:
                continue
        return nodes
    except Exception as e:
        _log(f"fetch_vless_nodes error ({url}): {e}"); return []

class exitfyPlugin(BasePlugin):

    # === mkStats: integration start ===
    def _mkstats_get_setting(self, key: str, default):
        try:
            if hasattr(self, "get_setting"):
                return self.get_setting(key, default)
            if hasattr(self, "getsetting"):
                return self.getsetting(key, default)
        except Exception:
            pass
        return default

    def _mkstats_set_setting(self, key: str, value, reload_settings: bool = False):
        try:
            if hasattr(self, "set_setting"):
                return self.set_setting(key, value, reload_settings=reload_settings)
            if hasattr(self, "setsetting"):
                return self.setsetting(key, value, reloadsettings=reload_settings)
        except Exception:
            pass
        return None

    def _mkstats_get_device_id(self) -> str:
        device_id = self._mkstats_get_setting("mkstats_device_id", "")
        if not device_id:
            device_id = uuid.uuid4().hex
            self._mkstats_set_setting("mkstats_device_id", device_id, reload_settings=False)
        return device_id

    def _mkstats_get_client_version(self) -> str:
        try:
            from org.telegram.messenger import BuildVars
            version = getattr(BuildVars, "BUILD_VERSION_STRING", None) or getattr(BuildVars, "BUILD_VERSION", None)
            if version:
                return str(version)
        except Exception:
            pass
        try:
            from org.telegram.messenger import BuildConfig as TgBuildConfig
            version = getattr(TgBuildConfig, "VERSION_NAME", None)
            if version:
                return str(version)
        except Exception:
            pass
        try:
            from com.radolyn.ayugram import BuildConfig as AyuBuildConfig
            version = getattr(AyuBuildConfig, "VERSION_NAME", None)
            if version:
                return str(version)
        except Exception:
            pass
        try:
            from com.exteragram.messenger import BuildConfig as ExBuildConfig
            version = getattr(ExBuildConfig, "VERSION_NAME", None)
            if version:
                return str(version)
        except Exception:
            pass
        try:
            from org.telegram.messenger import ApplicationLoader
            ctx = ApplicationLoader.applicationContext
            if ctx:
                pm = ctx.getPackageManager()
                pkg = ctx.getPackageName()
                info = pm.getPackageInfo(pkg, 0)
                version = getattr(info, "versionName", None) or getattr(info, "versionCode", None)
                if version:
                    return str(version)
        except Exception:
            pass
        return "unknown"

    def _mkstats_get_client_name(self) -> str:
        try:
            from org.telegram.messenger import ApplicationLoader
            ctx = ApplicationLoader.applicationContext
            if ctx:
                pkg = ctx.getPackageName()
                if pkg == "com.radolyn.ayugram":
                    return "AyuGram"
                if pkg == "com.exteragram.messenger":
                    return "exteraGram"
        except Exception:
            pass
        try:
            from com.radolyn.ayugram import BuildConfig as AyuBuildConfig
            _ = AyuBuildConfig.VERSION_NAME
            return "AyuGram"
        except Exception:
            pass
        try:
            from com.exteragram.messenger import BuildConfig as ExBuildConfig
            _ = ExBuildConfig.VERSION_NAME
            return "exteraGram"
        except Exception:
            pass
        return "unknown"

    def _mkstats_log(self, message: str) -> None:
        if hasattr(self, "log"):
            try:
                self.log(message)
            except Exception:
                pass

    def _mkstats_track(self, event: str, count: int = 1, cooldown_sec: int = 60) -> None:
        try:
            if not event:
                return
            try:
                event = str(event)
            except Exception:
                return
            try:
                c = int(count)
            except Exception:
                c = 1
            if c < 1:
                c = 1
            if c > 1000:
                c = 1000
            now = int(time.time())
            try:
                cd = int(cooldown_sec)
            except Exception:
                cd = 60
            if cd < 0:
                cd = 0
            try:
                if not hasattr(self, "_mkstats_last_event") or self._mkstats_last_event is None:
                    self._mkstats_last_event = {}
                last = int(self._mkstats_last_event.get(event, 0) or 0)
                if cd and (now - last) < cd:
                    return
                self._mkstats_last_event[event] = now
            except Exception:
                pass
            self._mkstats_event(event, count=c)
        except Exception:
            pass

    def _mkstats_event(self, event: str, count: int = 1) -> None:
        if not event:
            return

        def _send():
            try:
                if not hasattr(self, "_mkstats_client"):
                    return
                try:
                    if not bool(getattr(self, "_mkstats_initialized", False)):
                        return
                except Exception:
                    return
                if not getattr(self, "_mkstats_token", ""):
                    data = self._mkstats_client.handshake()
                    self._mkstats_token = data.get("install_token", "")
                    if self._mkstats_token:
                        self._mkstats_set_setting("mkstats_install_token", self._mkstats_token, reload_settings=False)
                if self._mkstats_token:
                    resp = self._mkstats_client.send_event(self._mkstats_token, event, count=count)
                    st = int((resp or {}).get("_http_status") or 0)
                    if st == 401:
                        self._mkstats_token = ""
                        self._mkstats_set_setting("mkstats_install_token", "", reload_settings=False)
            except Exception as exc:
                self._mkstats_log(f"mkStats: event error {exc}")
                self._mkstats_token = ""
                self._mkstats_set_setting("mkstats_install_token", "", reload_settings=False)

        try:
            threading.Thread(target=_send, daemon=True).start()
        except Exception:
            pass

    def _mkstats_loop(self):
        backoff = 0
        while not self._mkstats_stop.is_set():
            try:
                if not self._mkstats_token:
                    self._mkstats_log("mkStats: handshake start")
                    data = self._mkstats_client.handshake()
                    self._mkstats_token = data.get("install_token", "")
                    if self._mkstats_token:
                        self._mkstats_set_setting("mkstats_install_token", self._mkstats_token, reload_settings=False)
                        self._mkstats_log("mkStats: handshake ok, token stored")
                    else:
                        self._mkstats_log("mkStats: handshake response missing token")

                if self._mkstats_token:
                    self._mkstats_log("mkStats: sending ping")
                    resp = self._mkstats_client.send_ping(self._mkstats_token)
                    st = int((resp or {}).get("_http_status") or 0)
                    if st == 200:
                        try:
                            self._mkstats_initialized = True
                        except Exception:
                            pass
                        backoff = 0
                    elif st == 401:
                        self._mkstats_token = ""
                        self._mkstats_set_setting("mkstats_install_token", "", reload_settings=False)
                        try:
                            self._mkstats_initialized = False
                        except Exception:
                            pass
                    elif st == 429:
                        backoff = min(6 * 3600, max(60, (backoff * 2) if backoff else 60))
                    self._mkstats_log("mkStats: ping sent")
            except Exception as exc:
                self._mkstats_log(f"mkStats: error {exc}")
                self._mkstats_token = ""
                self._mkstats_set_setting("mkstats_install_token", "", reload_settings=False)
            try:
                jitter = random.randint(0, 60)
            except Exception:
                jitter = 0
            wait_s = int(MKSTATS_PING_INTERVAL) + int(jitter)
            if backoff:
                wait_s = int(backoff)
            self._mkstats_stop.wait(wait_s)

    def _mkstats_start(self):
        try:
            device_id = self._mkstats_get_device_id()
            user_hash = generate_user_hash(device_id, __id__)
            device_fingerprint = generate_device_fingerprint(device_id)
            client_name = self._mkstats_get_client_name()
            client_version = self._mkstats_get_client_version()
            self._mkstats_client = MkStatsCoreClient(MKSTATS_API_URL, __id__, __version__, user_hash, device_fingerprint, client_version, client_name)
            self._mkstats_stop = threading.Event()
            self._mkstats_token = self._mkstats_get_setting("mkstats_install_token", "")
            self._mkstats_initialized = False
            self._mkstats_last_event = {}
            self._mkstats_thread = threading.Thread(target=self._mkstats_loop, daemon=True)
            self._mkstats_thread.start()
            self._mkstats_log(f"mkStats: client started ({self._mkstats_client.api_base})")
        except Exception:
            pass
    # === mkStats: integration end ===

    def on_plugin_load(self):

        # === mkStats: integration start ===
        self._mkstats_start()
        # === mkStats: integration end ===

        self._vless_core = VlessCore()
        self._vless_port = 20808
        self._vless_running = False
        self._ping_event = threading.Event()
        self._ping_stop = threading.Event()
        self._health_stop = threading.Event()
        self._health_fail_count = 0
        self._last_clipboard_uri = ""
        self._chat_menu_item_id = "exitfy_settings_chat"
        self._drawer_menu_item_id = "exitfy_settings_drawer"

        self._proxy_links = []
        self._proxy_cycle_idx = 0
        self._watchdog_stop = threading.Event()

        self._vpn_guard_stop = threading.Event()
        self._vpn_guard_was_active = False
        self._vpn_guard_restore_state = None

        self._tgws_core = TgWsCore()
        self._tgws_port = 0
        self._tgws_running = False
        self._tgws_proxy_snapshot = None
        self._tgws_proxy_managed = False

        self._pill_id = _to_int(self.get_setting("pill_id", 9996), 9996)
        self._pill_registered = False
        self._pill_mode = None
        self._pill_bootstrapped = False
        self._pill_instances = {}
        self._pill_system = None

        self._vless_data = self.get_setting("vless_data", {"manual": [], "subs": [], "active_uri": ""})
        self._vless_active_uri = self._vless_data.get("active_uri", "")
        self._vless_custom_sub_url = self.get_setting("vless_custom_sub_url", "")
        self._ping_stop = threading.Event()
        self._vless_init_in_progress = False
        self._last_custom_key_applied = ""
        self._last_custom_sub_applied = ""

        try:
            self._proxy_custom_urls = json.loads(_pget_str("proxy_custom_urls", "[]"))
            if not isinstance(self._proxy_custom_urls, list):
                self._proxy_custom_urls = []
        except:
            self._proxy_custom_urls = []

        self._add_menu_items()

        try:
            self._install_voip_hook()
        except:
            pass

        try:
            self._mkstats_event("plugin_load")
        except:
            pass

        try:
            self._pill_bootstrap()
        except:
            pass

        self._vpn_guard_stop.clear()
        try:
            if bool(self.get_setting("vpn_guard_enabled", True)):
                threading.Thread(target=self._vpn_guard_loop, daemon=True).start()
        except:
            threading.Thread(target=self._vpn_guard_loop, daemon=True).start()

        mode = self.get_setting("mode", 1)
        if mode == 0:
            if self.get_setting("proxy_enabled", False):
                last_link = _pget_str("last_proxy_link", "")
                if last_link:
                    info = _parse_link(last_link)
                    if info:
                        _apply_proxy_info(info)
                threading.Thread(target=self._watchdog_loop, daemon=True).start()
        elif mode == 1:
            threading.Thread(target=self._vless_pinger_loop, daemon=True).start()
            threading.Thread(target=self._vless_health_check_loop, daemon=True).start()
            try:
                self._install_clipboard_hook()
            except:
                pass
            if self.get_setting("vless_enabled", False) and self._vless_active_uri:
                threading.Thread(target=self._vless_connect, daemon=True).start()
        else:
            if self.get_setting("proxy_enabled", False):
                threading.Thread(target=self._tgws_connect_bg, daemon=True).start()

    def on_plugin_unload(self):

        # === mkStats: integration start ===
        if hasattr(self, "_mkstats_stop"):
            self._mkstats_stop.set()
            self._mkstats_log("mkStats: stop requested")
            try:
                if hasattr(self, "_mkstats_thread") and self._mkstats_thread is not None:
                    self._mkstats_thread.join(timeout=1.0)
            except Exception:
                pass
        # === mkStats: integration end ===

        self._watchdog_stop.set()
        self._ping_stop.set()
        self._health_stop.set()
        self._vpn_guard_stop.set()
        try:
            _was_tgws = bool(getattr(self, "_tgws_running", False) or getattr(self, "_tgws_proxy_managed", False))
        except Exception:
            _was_tgws = False
        try:
            self._tgws_disconnect_with_reason("plugin_unload", silent=True)
        except:
            pass
        if not _was_tgws:
            self._vless_disconnect(silent=True)
        self._remove_menu_items()
        try:
            self._mkstats_event("plugin_unload")
        except:
            pass

    def on_app_event(self, event_type):
        try:
            AppEvent = find_class("com.exteragram.messenger.plugins.AppEvent")
            if not AppEvent:
                return
            if event_type in (AppEvent.START, AppEvent.RESUME):
                try:
                    mode = _to_int(self.get_setting("mode", 1), 1)
                except Exception:
                    mode = 1
                if mode == 2 and bool(self.get_setting("proxy_enabled", False)):
                    if not bool(getattr(self, "_tgws_running", False)):
                        threading.Thread(target=self._tgws_connect_bg, daemon=True).start()
        except Exception:
            pass

    def _tgws_get_static_field(self, cls, name, default=None):
        try:
            f = cls.getDeclaredField(str(name))
            f.setAccessible(True)
            return f.get(None)
        except Exception:
            return default

    def _tgws_set_static_field(self, cls, name, value):
        try:
            f = cls.getDeclaredField(str(name))
            f.setAccessible(True)
            f.set(None, value)
        except Exception:
            pass

    def _tgws_snapshot_current_proxy(self):
        if self._tgws_proxy_snapshot is not None:
            return
        try:
            shared_config_cls = find_class("org.telegram.messenger.SharedConfig")
            current = self._tgws_get_static_field(shared_config_cls, "currentProxyInfo", None)
            enabled = bool(self._tgws_get_static_field(shared_config_cls, "useProxySettings", False))
            snapshot = {
                "enabled": enabled,
                "host": "",
                "port": 0,
                "user": "",
                "password": "",
                "secret": "",
            }
            if current:
                snapshot["host"] = str(get_private_field(current, "address") or "")
                snapshot["port"] = int(get_private_field(current, "port") or 0)
                snapshot["user"] = str(get_private_field(current, "username") or "")
                snapshot["password"] = str(get_private_field(current, "password") or "")
                snapshot["secret"] = str(get_private_field(current, "secret") or "")
            self._tgws_proxy_snapshot = snapshot
        except Exception as exc:
            try:
                _log(f"tgws proxy snapshot failed: {exc}")
            except Exception:
                pass

    def _tgws_restore_previous_proxy(self):
        if not bool(getattr(self, "_tgws_proxy_managed", False)):
            return
        snapshot = getattr(self, "_tgws_proxy_snapshot", None)
        try:
            if snapshot is None:
                return
            enabled = bool(snapshot.get("enabled", False))
            host = str(snapshot.get("host", "") or "")
            port = int(snapshot.get("port", 0) or 0)
            user = str(snapshot.get("user", "") or "")
            password = str(snapshot.get("password", "") or "")
            secret = str(snapshot.get("secret", "") or "")

            shared_config_cls = find_class("org.telegram.messenger.SharedConfig")
            try:
                self._tgws_set_static_field(shared_config_cls, "useProxySettings", bool(enabled))
            except Exception:
                pass
            try:
                if host and port > 0:
                    ProxyInfo = jclass("org.telegram.messenger.SharedConfig$ProxyInfo")
                    pinfo = ProxyInfo(host, int(port), user, password, secret)
                    try:
                        self._tgws_set_static_field(shared_config_cls, "currentProxyInfo", pinfo)
                    except Exception:
                        pass
                    try:
                        SharedConfig.addProxy(pinfo)
                    except Exception:
                        pass
                else:
                    self._tgws_set_static_field(shared_config_cls, "currentProxyInfo", None)
            except Exception:
                pass

            try:
                ConnectionsManager.setProxySettings(bool(enabled), host, int(port), user, password, secret)
            except Exception:
                try:
                    for ac in range(UserConfig.MAX_ACCOUNT_COUNT):
                        cm = ConnectionsManager.getInstance(ac)
                        if cm:
                            cm.setProxySettings(bool(enabled), host, int(port), user, password, secret)
                except Exception:
                    pass
        finally:
            try:
                self._tgws_proxy_managed = False
            except Exception:
                pass
            try:
                self._tgws_proxy_snapshot = None
            except Exception:
                pass

    def _install_voip_hook(self):
        plugin_ref = self
        if jclass is None:
            return
        try:
            Class = jclass("java.lang.Class")
        except:
            return

        try:
            instance_cls = Class.forName("org.telegram.messenger.voip.Instance")
            proxy_cls = Class.forName("org.telegram.messenger.voip.Instance$Proxy")
        except:
            instance_cls = None
            proxy_cls = None

        if instance_cls is not None and proxy_cls is not None:
            try:
                ProxyCls = jclass("org.telegram.messenger.voip.Instance$Proxy")
            except:
                ProxyCls = None
            try:
                all_methods = instance_cls.getDeclaredMethods()
            except:
                all_methods = []

            try:
                for m in all_methods:
                    try:
                        mname = m.getName()
                        param_types = m.getParameterTypes()
                        proxy_arg_idx = -1
                        for i, pt in enumerate(param_types):
                            try:
                                pt_name = pt.getName()
                            except:
                                pt_name = ""
                            if "Proxy" in str(pt_name) and "[]" not in str(pt_name):
                                proxy_arg_idx = i
                                break
                        if proxy_arg_idx < 0:
                            continue

                        _idx = int(proxy_arg_idx)
                        _mname = str(mname)

                        class _ProxyInjectHook(MethodHook):
                            _pidx = _idx
                            _mn = _mname
                            def before_hooked_method(self, param):
                                try:
                                    if not bool(getattr(plugin_ref, "_vless_running", False)):
                                        return
                                    port = int(getattr(plugin_ref, "_vless_port", 0) or 0)
                                    if port <= 0:
                                        return
                                    if ProxyCls is None:
                                        return
                                    proxy = ProxyCls("127.0.0.1", int(port), "", "")
                                    param.args[self._pidx] = proxy
                                except:
                                    pass

                        self.hook_method(m, _ProxyInjectHook())
                    except:
                        continue
            except:
                pass

        try:
            voip_cls = None
            for cls_name in ("org.telegram.messenger.voip.VoIPService", "org.telegram.messenger.voip.VoIPBaseService"):
                try:
                    voip_cls = Class.forName(cls_name)
                    break
                except:
                    continue
            if voip_cls is not None:
                class _VoIPStartHook(MethodHook):
                    def before_hooked_method(self, param):
                        try:
                            if not bool(getattr(plugin_ref, "_vless_running", False)):
                                return
                            port = int(getattr(plugin_ref, "_vless_port", 0) or 0)
                            if port <= 0:
                                return
                            try:
                                SharedConfig.proxyEnabled = True
                            except:
                                pass
                            try:
                                ed = MessagesController.getGlobalMainSettings().edit()
                                ed.putBoolean("proxy_enabled_calls", True)
                                ed.putBoolean("proxy_calls_enabled", True)
                                ed.putBoolean("calls_use_proxy", True)
                                ed.commit()
                            except:
                                pass
                        except:
                            pass

                hook_targets = ("startCall", "startRinging", "acceptIncomingCall", "onCallUpdated", "startGroupCall")
                try:
                    for m in voip_cls.getDeclaredMethods():
                        try:
                            if m.getName() in hook_targets:
                                self.hook_method(m, _VoIPStartHook())
                        except:
                            continue
                except:
                    pass
        except:
            pass

    def _download_libvless(self):
        return self._download_libvless_with_progress(None)

    def _download_libvless_with_progress(self, on_progress):
        try:
            ctx = ApplicationLoader.applicationContext
            dir_path = os.path.join(str(ctx.getFilesDir()), "exitfy_modules")
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)
            so_path = os.path.join(dir_path, "libvless.so")
            url = "https://github.com/coidarashka/MandreLib-Modules/releases/download/LocalAiChat/libvless.so"

            _log("libvless download: start")
            try:
                self._mkstats_track("libvless_download_start", cooldown_sec=30)
            except:
                pass
            r = requests.get(url, stream=True, timeout=30)
            r.raise_for_status()

            total = 0
            try:
                total = int(r.headers.get("content-length") or 0)
            except:
                total = 0

            done = 0
            if callable(on_progress):
                try: on_progress(0.0)
                except: pass

            with open(so_path, "wb") as f:
                for chunk in r.iter_content(8192):
                    if not chunk:
                        continue
                    f.write(chunk)
                    done += len(chunk)
                    if callable(on_progress):
                        try:
                            if total > 0:
                                on_progress(float(done) / float(total))
                            else:
                                on_progress(0.0)
                        except:
                            pass

            if callable(on_progress):
                try: on_progress(1.0)
                except: pass

            _log(f"libvless download: done bytes={done} total={total}")
            try:
                self._mkstats_track("libvless_download_ok", cooldown_sec=30)
            except:
                pass
            try:
                if os.path.getsize(so_path) < 1024:
                    _log("libvless download: file too small")
                    return False
            except:
                return False
            return True
        except Exception as e:
            _log(f"libvless download error: {e}")
            try:
                self._mkstats_track("libvless_download_error", cooldown_sec=30)
            except:
                pass
            return False

    def _download_libvless_and_reload(self):
        try:
            frag = get_last_fragment()
            act = frag.getParentActivity() if frag else None
        except:
            act = None

        if not act:
            threading.Thread(target=lambda: self._download_libvless_and_reload_bg(None), daemon=True).start()
            return

        dialog_box = {"builder": None}

        def show_dialog():
            try:
                builder = AlertDialogBuilder(act, AlertDialogBuilder.ALERT_TYPE_SPINNER)
                try:
                    builder.set_title(Z("addon_download_title"))
                except:
                    pass
                try:
                    builder.set_message(Z("addon_downloading"))
                except:
                    pass
                builder.show()
                try:
                    builder.set_cancelable(False)
                except:
                    pass
                try:
                    builder.set_canceled_on_touch_outside(False)
                except:
                    pass

                dialog_box["builder"] = builder
            except Exception as e:
                _log(f"download dialog show error: {e}")

        def close_dialog():
            try:
                b = dialog_box.get("builder")
                if b:
                    b.dismiss()
            except:
                pass

        def set_progress(p):
            try:
                return
            except:
                pass

        run_on_ui_thread(show_dialog)

        def task():
            try:
                def on_progress(p):
                    run_on_ui_thread(lambda: set_progress(p))

                ok = self._download_libvless_with_progress(on_progress)
                if ok:
                    _log("libvless download: init core")
                    self._vless_core = VlessCore()
                    try:
                        _log(f"libvless core available={self._vless_core.is_available()} mode={getattr(self._vless_core, 'mode', None)}")
                    except:
                        pass
                run_on_ui_thread(close_dialog)
                run_on_ui_thread(self._reload_settings_ui)
                if not ok:
                    try:
                        run_on_ui_thread(lambda: BulletinHelper.show_error(Z("addon_download_error")))
                    except:
                        pass
            except Exception as e:
                _log(f"download libvless task error: {e}")
                run_on_ui_thread(close_dialog)
                run_on_ui_thread(self._reload_settings_ui)
                try:
                    run_on_ui_thread(lambda: BulletinHelper.show_error(Z("addon_download_error")))
                except:
                    pass

        threading.Thread(target=task, daemon=True).start()

    def _download_libvless_and_reload_bg(self, on_progress):
        try:
            ok = self._download_libvless_with_progress(on_progress)
            if ok:
                self._vless_core = VlessCore()
            run_on_ui_thread(self._reload_settings_ui)
        except Exception as e:
            _log(f"download libvless bg error: {e}")
            run_on_ui_thread(self._reload_settings_ui)

    def _libvless_so_path(self):
        try:
            ctx = ApplicationLoader.applicationContext
            dir_path = os.path.join(str(ctx.getFilesDir()), "exitfy_modules")
            return os.path.join(dir_path, "libvless.so")
        except:
            return None

    def _has_libvless_addon(self):
        try:
            p = self._libvless_so_path()
            return bool(p) and os.path.exists(p) and os.path.getsize(p) >= 1024
        except:
            return False

    def _delete_libvless_and_reload(self):
        try:
            self._vless_disconnect(silent=True)
        except:
            pass

        def task():
            ok = False
            try:
                p = self._libvless_so_path()
                if p and os.path.exists(p):
                    try:
                        os.remove(p)
                        ok = True
                    except Exception as e:
                        _log(f"libvless delete error: {e}")
                self._vless_core = VlessCore()
            except Exception as e:
                _log(f"libvless delete task error: {e}")
            run_on_ui_thread(self._reload_settings_ui)
            if not ok:
                try:
                    run_on_ui_thread(lambda: BulletinHelper.show_error(Z("addon_delete_error")))
                except:
                    pass

        threading.Thread(target=task, daemon=True).start()

    def _is_system_vpn_active(self):
        try:
            context = ApplicationLoader.applicationContext
            cm = context.getSystemService(Context.CONNECTIVITY_SERVICE)
            if cm:
                try:
                    networks = cm.getAllNetworks()
                    for net in networks:
                        caps = cm.getNetworkCapabilities(net)
                        if caps and caps.hasTransport(NetworkCapabilities.TRANSPORT_VPN):
                            try:
                                if caps.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET):
                                    return True
                            except:
                                return True
                except:
                    pass

            try:
                cm2 = context.getSystemService("connectivity")
            except:
                cm2 = None

            if cm2:
                try:
                    active = cm2.getActiveNetwork()
                    caps = cm2.getNetworkCapabilities(active) if active else None
                    if caps:
                        return bool(caps.hasTransport(NetworkCapabilities.TRANSPORT_VPN))
                except:
                    pass

                try:
                    info = cm2.getActiveNetworkInfo()
                    if info and info.isConnected():
                        try:
                            from android.net import ConnectivityManager
                            return int(info.getType()) == int(ConnectivityManager.TYPE_VPN)
                        except:
                            pass
                except:
                    pass
        except:
            return False
        return False

    def _disable_plugin_due_to_vpn(self):
        try:
            mode = _to_int(self.get_setting("mode", 1), 1)

            try:
                self._vpn_guard_restore_state = {
                    "mode": mode,
                    "proxy_enabled": bool(self.get_setting("proxy_enabled", False)),
                    "vless_enabled": bool(self.get_setting("vless_enabled", False)),
                }
            except:
                self._vpn_guard_restore_state = None

            self._watchdog_stop.set()
            if mode == 1:
                self._vless_disconnect(silent=True)
                try:
                    self.set_setting("vless_enabled", False, reload_settings=True)
                except:
                    pass
            else:
                try:
                    self.set_setting("proxy_enabled", False, reload_settings=True)
                except:
                    pass

            _set_tg_proxy_enabled(False)
            try:
                _vless_apply_tg_proxy(0, False)
            except:
                pass

            run_on_ui_thread(self._reload_settings_ui)
            try:
                self._pill_update()
            except:
                pass
            try:
                self._mkstats_track("vpn_guard_disabled", cooldown_sec=30)
            except:
                pass
            _log("system VPN detected -> plugin disabled")
        except Exception as e:
            _log(f"vpn auto-disable error: {e}")

    def _restore_plugin_after_vpn(self):
        try:
            st = getattr(self, "_vpn_guard_restore_state", None)
            if not isinstance(st, dict):
                return
            mode = _to_int(st.get("mode", _to_int(self.get_setting("mode", 1), 1)), 1)
            if mode == 1:
                if bool(st.get("vless_enabled", False)):
                    try:
                        self.set_setting("vless_enabled", True, reload_settings=True)
                    except:
                        pass
                    if self._vless_active_uri:
                        threading.Thread(target=self._vless_connect, daemon=True).start()
            else:
                if bool(st.get("proxy_enabled", False)):
                    try:
                        self.set_setting("proxy_enabled", True, reload_settings=True)
                    except:
                        pass
                    last_link = _pget_str("last_proxy_link", "")
                    if last_link:
                        info = _parse_link(last_link)
                        if info:
                            _apply_proxy_info(info)
                    _set_tg_proxy_enabled(True)
                    self._watchdog_stop.clear()
                    threading.Thread(target=self._watchdog_loop, daemon=True).start()
            self._vpn_guard_restore_state = None
            run_on_ui_thread(self._reload_settings_ui)
            try:
                self._pill_update()
            except:
                pass
            try:
                self._mkstats_track("vpn_guard_restored", cooldown_sec=30)
            except:
                pass
            _log("system VPN off -> plugin state restored")
        except Exception as e:
            _log(f"vpn restore error: {e}")

    def _pill_is_enabled(self):
        try:
            mode = _to_int(self.get_setting("mode", 1), 1)
            if mode == 1:
                return bool(self.get_setting("vless_enabled", False))
            return bool(self.get_setting("proxy_enabled", False))
        except:
            return False

    def _pill_toggle(self):
        try:
            mode = _to_int(self.get_setting("mode", 1), 1)
            if mode == 1:
                cur = bool(self.get_setting("vless_enabled", False))
                nv = (not cur)
                try:
                    self.set_setting("vless_enabled", nv, reload_settings=True)
                except:
                    pass
                self._on_enabled_changed(nv)
                try:
                    self._mkstats_track("pill_toggle_on" if nv else "pill_toggle_off", cooldown_sec=2)
                except:
                    pass
            else:
                cur = bool(self.get_setting("proxy_enabled", False))
                nv = (not cur)
                try:
                    self.set_setting("proxy_enabled", nv, reload_settings=True)
                except:
                    pass
                self._on_enabled_changed(nv)
                try:
                    self._mkstats_track("pill_toggle_on" if nv else "pill_toggle_off", cooldown_sec=2)
                except:
                    pass
            self._pill_update()
        except:
            pass

    def _pill_open_settings(self):
        try:
            self._open_settings()
        except:
            pass

    def _pill_get_custom_pill(self):
        try:
            import sys
            return sys.modules.get("custom_pill")
        except:
            return None

    def _pill_bootstrap(self):
        if getattr(self, "_pill_bootstrapped", False):
            return
        self._pill_bootstrapped = True
        try:
            run_on_ui_thread(self._pill_register_or_retry, 200)
        except Exception:
            self._pill_register_or_retry()

    def _pill_prefs(self):
        try:
            ctx = ApplicationLoader.applicationContext
            if not ctx:
                return None
            return ctx.getSharedPreferences("exitfy_pill", 0)
        except Exception:
            return None

    def _pill_get_saved_state(self):
        try:
            prefs = self._pill_prefs()
            if not prefs:
                return -1
            return int(prefs.getInt("active_state", -1))
        except Exception:
            return -1

    def _pill_set_saved_state(self, state):
        try:
            prefs = self._pill_prefs()
            if not prefs:
                return
            prefs.edit().putInt("active_state", int(state)).apply()
        except Exception:
            pass

    def _pill_get_saved_index(self):
        try:
            prefs = self._pill_prefs()
            if not prefs:
                return -1
            return int(prefs.getInt("active_index", -1))
        except Exception:
            return -1

    def _pill_set_saved_index(self, idx):
        try:
            prefs = self._pill_prefs()
            if not prefs:
                return
            prefs.edit().putInt("active_index", int(idx)).apply()
        except Exception:
            pass

    def _pill_java_int(self, v):
        try:
            return Integer(int(v)) if Integer else int(v)
        except Exception:
            return int(v)

    def _pill_active_index(self, active, pid_obj):
        try:
            idx = active.indexOf(pid_obj)
            if idx is not None and int(idx) >= 0:
                return int(idx)
        except Exception:
            pass
        try:
            size = int(active.size())
            for i in range(size):
                try:
                    if int(active.get(i)) == int(pid_obj):
                        return int(i)
                except Exception:
                    continue
        except Exception:
            pass
        return -1

    def _pill_place_in_active(self, active, pid_obj, idx):
        try:
            active.remove(pid_obj)
        except Exception:
            pass
        try:
            size = int(active.size())
        except Exception:
            size = -1
        if idx is None or idx < 0:
            try:
                active.add(pid_obj)
            except Exception:
                pass
            return
        try:
            if size >= 0 and idx > size:
                idx = size
            active.add(int(idx), pid_obj)
        except Exception:
            try:
                active.add(pid_obj)
            except Exception:
                pass

    def _pill_sync_state_from_config(self):
        try:
            PillStackConfig = find_class("com.exteragram.messenger.pillstack.core.PillStackConfig")
            active = getattr(PillStackConfig, "activePills", None)
            hidden = getattr(PillStackConfig, "hiddenPills", None)
            if active is None or hidden is None:
                return
            pid_obj = self._pill_java_int(self._pill_id)
            try:
                if active.contains(pid_obj):
                    self._pill_set_saved_state(1)
                    try:
                        idx = self._pill_active_index(active, pid_obj)
                        if idx >= 0:
                            self._pill_set_saved_index(idx)
                    except Exception:
                        pass
                    return
            except Exception:
                pass
            try:
                if hidden.contains(pid_obj):
                    self._pill_set_saved_state(0)
                    return
            except Exception:
                pass
        except Exception:
            pass

    def _pill_restore_visibility(self):
        try:
            state = self._pill_get_saved_state()
            if state not in (0, 1):
                return
            PillStackConfig = find_class("com.exteragram.messenger.pillstack.core.PillStackConfig")
            try:
                PillStackConfig.loadConfig()
            except Exception:
                pass
            active = getattr(PillStackConfig, "activePills", None)
            hidden = getattr(PillStackConfig, "hiddenPills", None)
            if active is None or hidden is None:
                return
            pid_obj = self._pill_java_int(self._pill_id)
            idx = self._pill_get_saved_index()
            changed = False
            if state == 1:
                try:
                    if hidden.contains(pid_obj):
                        hidden.remove(pid_obj)
                        changed = True
                except Exception:
                    pass
                try:
                    if not active.contains(pid_obj):
                        self._pill_place_in_active(active, pid_obj, idx)
                        changed = True
                    else:
                        if idx is not None and idx >= 0:
                            cur = self._pill_active_index(active, pid_obj)
                            if cur != idx:
                                self._pill_place_in_active(active, pid_obj, idx)
                                changed = True
                except Exception:
                    pass
            else:
                try:
                    if active.contains(pid_obj):
                        active.remove(pid_obj)
                        changed = True
                except Exception:
                    pass
                try:
                    if not hidden.contains(pid_obj):
                        hidden.add(pid_obj)
                        changed = True
                except Exception:
                    pass
            if changed:
                try:
                    PillStackConfig.savePillsLayout()
                except Exception:
                    pass
                try:
                    PillStackConfig.notifySettingsChanged()
                except Exception:
                    pass
        except Exception:
            pass

    def _pill_register_or_retry(self):
        if getattr(self, "_pill_mode", None):
            return
        mod = self._pill_get_custom_pill()
        register = getattr(mod, "register_external_pill", None) if mod else None
        updater = getattr(mod, "update_external_pill", None) if mod else None
        if not register or not updater:
            if self._pill_register_standalone():
                self._pill_mode = "standalone"
                try:
                    self._pill_sync_state_from_config()
                except:
                    pass
                try:
                    self._pill_restore_visibility()
                except Exception:
                    pass
                try:
                    self._pill_update_standalone()
                except:
                    pass
                return
            run_on_ui_thread(self._pill_register_or_retry, 1200)
            return
        try:
            R_drawable = find_class("org.telegram.messenger.R$drawable")
            icon_res = getattr(R_drawable, "msg_secret", getattr(R_drawable, "msg_proxy_on", getattr(R_drawable, "msg_settings", 0)))
        except:
            icon_res = 0
        try:
            self._pill_icon_res = int(icon_res or 0)
        except:
            self._pill_icon_res = 0
        try:
            register(
                int(self._pill_id),
                "exitFy",
                "OFF",
                icon_res=int(icon_res or 0),
                color_top=Color.parseColor("#F44336"),
                color_bottom=Color.parseColor("#D32F2F"),
                on_click=self._pill_toggle,
                on_long_click=self._pill_open_settings,
                icon_tint=Color.WHITE,
                text_size_sp=12,
                min_width_dp=120,
            )
            self._pill_registered = True
            self._pill_mode = "custom"
            try:
                _log(f"pill: registered id={int(self._pill_id)} via custom_pill")
                self._pill_restore_visibility()
                self._pill_ensure_visible(self._pill_id)
                self._pill_sync_pillstack()
                self._pill_sync_state_from_config()
                self._pill_update()
            except:
                pass
        except:
            run_on_ui_thread(self._pill_register_or_retry, 1200)

    def _pill_register_standalone(self):
        try:
            if dynamic_proxy is None:
                return False
            PillRegistry = find_class("com.exteragram.messenger.pillstack.core.PillRegistry")
            PillInfo = find_class("com.exteragram.messenger.pillstack.core.PillRegistry$PillInfo")
            CachePill = find_class("com.exteragram.messenger.pillstack.ui.pills.system.CachePill")
            PillCreatorInterface = find_class("com.exteragram.messenger.pillstack.core.PillRegistry$PillCreator")

            try:
                self._pill_install_hooks()
            except Exception:
                pass

            try:
                for pid in (int(self._pill_id), 9996, 9997, 9998, 9995, 9994):
                    try:
                        if not PillRegistry.isRegistered(int(pid)):
                            self._pill_id = int(pid)
                            break
                    except Exception:
                        continue
            except Exception:
                pass

            class _Creator(dynamic_proxy(PillCreatorInterface)):
                def __init__(self, plugin):
                    super().__init__()
                    self._p = plugin
                def create(self, context, resourcesProvider):
                    pill = CachePill(context, resourcesProvider)
                    try:
                        pill.setTag(Integer(int(self._p._pill_id)) if Integer else int(self._p._pill_id))
                    except Exception:
                        pass
                    try:
                        pill.setTag(int(self._p._pill_id), Integer(int(self._p._pill_id)) if Integer else int(self._p._pill_id))
                    except Exception:
                        pass
                    try:
                        self._p._pill_apply_visuals_standalone(pill)
                    except Exception:
                        pass
                    try:
                        self._p._pill_remember_instance(pill)
                    except Exception:
                        pass
                    return pill

            try:
                R_drawable = find_class("org.telegram.messenger.R$drawable")
                icon_res = getattr(R_drawable, "msg_secret", getattr(R_drawable, "msg_proxy_on", getattr(R_drawable, "msg_settings", 0)))
            except Exception:
                icon_res = 0
            self._pill_icon_res = int(icon_res or 0)

            creator = _Creator(self)
            info = PillInfo(int(self._pill_id), "exitFy", int(self._pill_icon_res or 0), int(Color.parseColor("#9E9E9E")), int(Color.parseColor("#616161")), creator)
            PillRegistry.register(info)
            self._pill_registered = True
            try:
                self._pill_ensure_visible(self._pill_id)
                self._pill_sync_pillstack()
            except Exception:
                pass
            return True
        except Exception:
            return False

    def _pill_install_hooks(self):
        if not jclass:
            return
        try:
            JClass = jclass("java.lang.Class")
            CachePillClass = JClass.forName("com.exteragram.messenger.pillstack.ui.pills.system.CachePill")
            for method in CachePillClass.getDeclaredMethods():
                name = method.getName()
                if name == "onPillClicked":
                    self.hook_method(method, self._ExitFyPillClickHook(self))
                elif name == "onPillLongClicked":
                    self.hook_method(method, self._ExitFyPillLongClickHook(self))
                elif name == "onUpdateData":
                    self.hook_method(method, self._ExitFyPillUpdateHook(self))
                elif name == "onAttachedToWindow":
                    self.hook_method(method, self._ExitFyPillAttachHook(self))
        except Exception:
            pass

    def _pill_key(self, obj):
        try:
            if self._pill_system is None and jclass:
                self._pill_system = jclass("java.lang.System")
            if self._pill_system is not None:
                return int(self._pill_system.identityHashCode(obj))
        except Exception:
            pass
        try:
            return int(obj.hashCode())
        except Exception:
            return id(obj)

    def _pill_remember_instance(self, pill):
        try:
            self._pill_instances[self._pill_key(pill)] = pill
        except Exception:
            pass

    def _pill_is_ours(self, pill):
        try:
            if self._pill_key(pill) in self._pill_instances:
                return True
        except Exception:
            pass
        try:
            tag = pill.getTag()
            if tag is not None and int(tag) == int(self._pill_id):
                return True
        except Exception:
            pass
        try:
            tag2 = pill.getTag(int(self._pill_id))
            if tag2 is not None:
                return True
        except Exception:
            pass
        return False

    def _pill_apply_visuals_standalone(self, pill):
        try:
            cls = pill.getClass()
            f_text = cls.getDeclaredField("textView"); f_text.setAccessible(True)
            f_icon = cls.getDeclaredField("iconView"); f_icon.setAccessible(True)
            try:
                f_layout = cls.getDeclaredField("layout"); f_layout.setAccessible(True)
            except Exception:
                f_layout = None
            tv = f_text.get(pill)
            ic = f_icon.get(pill)
            layout = f_layout.get(pill) if f_layout else None
            enabled = False
            try:
                enabled = bool(self._pill_is_enabled())
            except Exception:
                enabled = False
            txt = f"exitFy · {'ON' if enabled else 'OFF'}"
            try:
                top = Color.parseColor("#4CAF50") if enabled else Color.parseColor("#9E9E9E")
                bot = Color.parseColor("#388E3C") if enabled else Color.parseColor("#616161")
            except Exception:
                top = 0
                bot = 0
            if layout:
                try:
                    layout.setVisibility(0)
                    ColoredBackground = find_class("com.exteragram.messenger.pillstack.ui.pills.crypto.utils.ColoredBackground")
                    layout.setBackground(ColoredBackground(int(top), int(bot)))
                except Exception:
                    pass
            if tv:
                try:
                    tv.setText(str(txt))
                except Exception:
                    pass
                try:
                    tv.setTextColor(int(Color.WHITE))
                except Exception:
                    pass
                try:
                    tv.setTextSize(12.0)
                except Exception:
                    pass
            if ic and self._pill_icon_res:
                try:
                    ic.setImageResource(int(self._pill_icon_res))
                except Exception:
                    pass
                try:
                    ic.setColorFilter(Color.WHITE)
                except Exception:
                    pass
        except Exception:
            pass

    def _pill_update_standalone(self):
        try:
            PillRegistry = find_class("com.exteragram.messenger.pillstack.core.PillRegistry")
            info = PillRegistry.getPillInfo(int(self._pill_id))
            if info is None:
                return
            enabled = bool(self._pill_is_enabled())
            name = f"exitFy · {'ON' if enabled else 'OFF'}"
            try:
                info.setName(str(name))
            except Exception:
                pass
            try:
                top = Color.parseColor("#4CAF50") if enabled else Color.parseColor("#9E9E9E")
                bot = Color.parseColor("#388E3C") if enabled else Color.parseColor("#616161")
                info.setTopColor(int(top)); info.setBottomColor(int(bot))
            except Exception:
                pass
            try:
                PillRegistry.notifyUpdated(int(self._pill_id))
            except Exception:
                pass
            try:
                for inst in list(getattr(self, "_pill_instances", {}).values()):
                    try:
                        self._pill_apply_visuals_standalone(inst)
                    except Exception:
                        pass
            except Exception:
                pass
        except Exception:
            pass

    def _on_vpn_guard_changed(self, enabled):
        try:
            self.set_setting("vpn_guard_enabled", bool(enabled), reload_settings=True)
        except:
            pass
        try:
            if not bool(enabled):
                self._vpn_guard_was_active = False
        except:
            pass

    def _pill_ensure_visible(self, pill_id):
        try:
            PillStackConfig = find_class("com.exteragram.messenger.pillstack.core.PillStackConfig")
            try:
                PillStackConfig.loadConfig()
            except:
                pass
            active = getattr(PillStackConfig, "activePills", None)
            hidden = getattr(PillStackConfig, "hiddenPills", None)
            if active is None or hidden is None:
                return
            try:
                pid_obj = Integer(int(pill_id)) if Integer else int(pill_id)
            except:
                pid_obj = int(pill_id)
            try:
                if (not active.contains(pid_obj)) and (not hidden.contains(pid_obj)):
                    hidden.add(pid_obj)
            except:
                try:
                    if (int(pill_id) not in list(active)) and (int(pill_id) not in list(hidden)):
                        hidden.add(pid_obj)
                except:
                    pass
            try:
                PillStackConfig.savePillsLayout()
            except:
                pass
            try:
                PillStackConfig.notifySettingsChanged()
            except:
                pass
        except:
            pass

    def _pill_sync_pillstack(self):
        try:
            PillStackConfig = find_class("com.exteragram.messenger.pillstack.core.PillStackConfig")
            try:
                PillStackConfig.sanitizePills()
            except:
                pass
            try:
                PillStackConfig.savePillsLayout()
            except:
                pass
            try:
                PillStackConfig.notifySettingsChanged()
            except:
                pass
        except:
            pass

    def _pill_update(self):
        try:
            mod = self._pill_get_custom_pill()
            updater = getattr(mod, "update_external_pill", None) if mod else None
            if not updater:
                if getattr(self, "_pill_mode", None) == "standalone":
                    self._pill_update_standalone()
                return
            enabled = bool(self._pill_is_enabled())
            try:
                mode = _to_int(self.get_setting("mode", 1), 1)
            except Exception:
                mode = 1

            if mode == 0:
                base = "Proxy"
            elif mode == 1:
                base = "VLESS"
            else:
                base = "TG WS"

            if enabled:
                txt = f"exitFy · {base} ON"
                top = Color.parseColor("#4CAF50")
                bot = Color.parseColor("#388E3C")
            else:
                txt = f"exitFy · {base} OFF"
                top = Color.parseColor("#9E9E9E")
                bot = Color.parseColor("#616161")

            updater(int(self._pill_id), txt, color_top=int(top), color_bottom=int(bot))
        except:
            pass

    def _tgws_connect_bg(self):
        ok = False
        try:
            ok = bool(self._tgws_connect())
        except Exception as e:
            _log(f"TGWS connect exception: {e}")
            try:
                self._tgws_last_error = str(e)
            except Exception:
                pass
            ok = False
        if not ok:
            msg = "TG WS не подключился"
            try:
                last = str(getattr(self, "_tgws_last_error", "") or "").strip()
                if last:
                    msg = f"TG WS не подключился: {last}"
            except Exception:
                pass
            try:
                run_on_ui_thread(lambda: BulletinHelper.show_error(str(msg)))
            except:
                pass
            try:
                self.set_setting("proxy_enabled", False, reload_settings=True)
            except:
                pass
            try:
                self._pill_update()
            except:
                pass
            try:
                run_on_ui_thread(self._reload_settings_ui)
            except:
                pass

    def _vpn_guard_loop(self):
        while not self._vpn_guard_stop.is_set():
            try:
                try:
                    if not bool(self.get_setting("vpn_guard_enabled", True)):
                        self._vpn_guard_stop.wait(3.0)
                        continue
                except:
                    pass
                vpn_active = bool(self._is_system_vpn_active())

                enabled_any = bool(self.get_setting("proxy_enabled", False)) or bool(self.get_setting("vless_enabled", False))
                if vpn_active and enabled_any:
                    if not self._vpn_guard_was_active:
                        self._vpn_guard_was_active = True
                    self._disable_plugin_due_to_vpn()
                elif not vpn_active:
                    if self._vpn_guard_was_active:
                        self._vpn_guard_was_active = False
                        self._restore_plugin_after_vpn()
            except:
                pass

            self._vpn_guard_stop.wait(3.0)

    def _init_default_vless_sub(self, provider=None):
        self._vless_init_in_progress = True
        if provider is None:
            provider = self.get_setting("vless_provider", 0)
        urls = VLESS_PROVIDER_SUBS.get(provider, list(VLESS_PROVIDER_SUBS.get(0, [])))
        for url in urls:
            nodes = _fetch_vless_nodes(url)
            if nodes:
                if "subs" not in self._vless_data: self._vless_data["subs"] = []
                existing_urls = [s["url"] for s in self._vless_data["subs"]]
                if url not in existing_urls:
                    self._vless_data["subs"].append({
                        "id": url, "url": url,
                        "name": urllib.parse.urlparse(url).netloc,
                        "nodes": nodes
                    })
                self._vless_save_data()
                self._ping_event.set()
        self.set_setting("_subs_loaded_for", provider)
        self._vless_init_in_progress = False
        self._reload_settings_ui()


    def create_settings(self):
        mode = _to_int(self.get_setting("mode", 1), 1)
        if mode == 0:
            en_key = "proxy_enabled"
        elif mode == 1:
            en_key = "vless_enabled"
        else:
            en_key = "proxy_enabled"
        en_text = Z("enable_plugin")
        items = [
            Header(text=Z("settings_header")),
            Switch(key=en_key, text=en_text,
                   default=False,
                   icon="msg_permissions", on_change=self._on_enabled_changed),
            Text(text=Z("open_tg_settings"),
                 icon="msg_settings",
                 on_click=lambda v: self._open_tg_proxy_settings()),
            Switch(
                key="vpn_guard_enabled",
                text="VPN Guard",
                default=True,
                subtext="Автоматически выключает прокси/VLESS, если включён системный VPN",
                icon="msg_settings",
                on_change=lambda v: self._on_vpn_guard_changed(bool(v)),
            ),
            Selector(key="mode", text=Z("select_mode"),
                     default=1,
                     items=[Z("mode_proxy"), Z("mode_vless"), Z("mode_tgws")],
                     icon="msg_switch", on_change=self._on_mode_changed),
            Divider(),
        ]

        try:
            if mode == 0:
                items += self._proxy_settings_items()
            elif mode == 1:
                items += self._vless_settings_items()
            else:
                items += self._tgws_settings_items()
        except Exception as e:
            _log(f"settings error: {e}")
            items += [Text(text=Z("settings_error"), icon="msg_info"), Divider()]

        try:
            del_btn = Z("addon_delete")

            def on_delete(v):
                try:
                    frag = get_last_fragment()
                    act = frag.getParentActivity() if frag else None
                    if not act:
                        self._delete_libvless_and_reload()
                        return

                    title = Z("addon_delete_title")
                    msg = Z("addon_delete_msg")
                    yes = Z("btn_delete")
                    no = Z("btn_cancel")

                    b = AlertDialogBuilder(act)
                    b.set_title(title)
                    b.set_message(msg)
                    b.set_negative_button(no, lambda bb, w: bb.dismiss())
                    b.set_positive_button(yes, lambda bb, w: (bb.dismiss(), self._delete_libvless_and_reload()))
                    try:
                        b.make_button_red(AlertDialogBuilder.BUTTON_POSITIVE)
                    except:
                        pass
                    b.show()
                except Exception as e:
                    _log(f"delete addon dialog error: {e}")
                    self._delete_libvless_and_reload()

            if self._has_libvless_addon():
                items.append(Divider())
                items.append(Text(text=del_btn, icon="msg_delete", red=True, on_click=on_delete))

        except:
            pass

        return items

    def _tgws_settings_items(self):
        items = []
        try:
            running = bool(getattr(self, "_tgws_running", False))
            st = "ON" if running else "OFF"
        except:
            st = "OFF"
        try:
            items.append(Text(text=f"TG WS · {st}", icon="msg_settings_14"))
        except:
            pass
        try:
            link = self._tgws_proxy_link()
            if link:
                items.append(Text(text=str(link), icon="msg_link"))
        except:
            pass
        items.append(Divider())
        return items

    def _proxy_settings_items(self):
        try:
            provider_idx = _to_int(self.get_setting("proxy_provider", 0), 0)
            auto_switch = self.get_setting("proxy_auto_switch", True)

            items = [
                Selector(key="proxy_provider", text=Z("select_provider"),
                         default=0,
                         items=[PROVIDER_NAMES[i] for i in sorted(PROVIDER_NAMES)],
                         icon="msg_channel", on_change=self._on_provider_changed),
            ]

            if not auto_switch:
                proxy_links = getattr(self, "_proxy_links", []) or []
                node_labels = []
                for i, info in enumerate([_parse_link(l) for l in proxy_links]):
                    node_labels.append(str(info.address) if info else f"{Z('proxy_node')} {i+1}")
                if node_labels:
                    items.append(Selector(key="proxy_manual_node", text=Z("node"),
                                          default=0,
                                          items=node_labels, icon="msg_settings_14",
                                          on_change=self._on_manual_node_changed))

            items.append(Text(text=Z("refresh"), icon="msg_retry",
                              on_click=lambda v: self._proxy_refresh()))

            items.append(Switch(key="proxy_auto_switch", text=Z("auto_switch"),
                                subtext=Z("auto_switch_desc"), default=True,
                                icon="media_flip", on_change=self._on_auto_switch_changed))

            if auto_switch:
                items += [
                    Header(text=Z("auto_switch_header")),
                    Input(key="proxy_connect_try", text=Z("connect_try"),
                          default=str(_pget_int("proxy_connect_try", 7)),
                          icon="msg_settings_14",
                          on_change=self._on_connect_try_changed),
                    Input(key="proxy_stuck_sec", text=Z("stuck"),
                          default=str(_pget_int("proxy_stuck_sec", 8)),
                          icon="msg_settings_14",
                          on_change=self._on_stuck_changed),
                    Input(key="proxy_cooldown_sec", text=Z("cooldown"),
                          default=str(_pget_int("proxy_cooldown_sec", 5)),
                          icon="msg_settings_14",
                          on_change=self._on_cooldown_changed),
                ]

            items += [
                Text(text=Z("referral_btn"), icon="msg_link",
                     accent=True,
                     on_click=self._make_referral_click(provider_idx, "proxy")),
                Divider(),
            ]

            for url in (getattr(self, "_proxy_custom_urls", []) or []):
                label = urllib.parse.urlparse(str(url)).netloc or str(url)
                items.append(Text(text=f"✕ {label}", icon="msg_delete",
                                  on_click=self._make_proxy_sub_delete(url)))

            items.append(Divider())
            return items
        except Exception as e:
            _log(f"proxy settings error: {e}")
            return [Text(text=Z("proxy_settings_error"), icon="msg_info"), Divider()]

    def _vless_settings_items(self):
        if not self._vless_core.is_available():
            msg = Z("vless_no_core_msg")
            btn = Z("vless_download_addon")
            items = [
                Divider(text=msg),
                Text(text=btn, icon="msg_download", accent=True,
                     on_click=lambda v: self._download_libvless_and_reload()),
            ]

            if self._has_libvless_addon():
                del_btn = Z("addon_delete")
                items.append(Text(text=del_btn, icon="msg_delete", red=True,
                                  on_click=lambda v: self._delete_libvless_and_reload()))
            items.append(Divider())
            return items

        active = self._vless_active_uri
        vless_provider = _to_int(self.get_setting("vless_provider", 0), 0)
        is_custom = (vless_provider == 3)
        sponsor_idx = min(vless_provider, 2) if not is_custom else -1

        if not is_custom and not self._vless_data.get("subs") and not self._vless_init_in_progress \
                and self.get_setting("_subs_loaded_for", -1) != vless_provider:
            self._vless_init_in_progress = True
            threading.Thread(target=self._init_default_vless_sub, args=(vless_provider,), daemon=True).start()

        items = [
            Selector(key="vless_provider", text=Z("vless_provider"),
                     default=0,
                     items=[PROVIDER_NAMES[i] for i in sorted(PROVIDER_NAMES)] + [Z("vless_prov_custom")],
                     icon="msg_channel", on_change=self._on_vless_provider_changed),
        ]

        try:
            if is_custom:
                pass
        except:
            pass

        if is_custom:
            items += [
                Input(key="vless_custom_key", text=Z("vless_add_key"),
                      default="",
                      icon="msg_link2",
                      on_change=self._on_vless_custom_key_input),
                Input(key="vless_custom_sub", text=Z("vless_add_sub"),
                      default="",
                      icon="msg_link",
                      on_change=self._on_vless_custom_sub_input),
            ]

        items += [
            Divider(),
            Header(text=Z("vless_nodes_header")),
        ]

        if not self._vless_data.get("manual") and not self._vless_data.get("subs"):
            items.append(Text(text=Z("vless_no_nodes")))
        else:
            seen = set()
            if self._vless_data.get("manual"):
                for node in self._vless_data["manual"]:
                    try:
                        u = _sanitize_vless_uri(node.get("uri", ""))
                        k = _uri_dedup_key(u)
                        if not u or not k or k in seen:
                            continue
                        seen.add(k)
                    except:
                        pass
                    is_act = (node["uri"] == active)
                    ping = node.get("ping", -1)
                    ping_str = f" — {ping}ms" if ping > 0 else ""
                    label = node["name"] + ping_str
                    items.append(Text(text=label, accent=bool(is_act),
                                      on_click=self._make_vless_node_select(node["uri"])))

            for sub in self._vless_data.get("subs", []):
                nodes = sub.get("nodes", [])
                if not nodes: continue
                unique_nodes = []
                for node in nodes:
                    try:
                        u = _sanitize_vless_uri(node.get("uri", ""))
                        k = _uri_dedup_key(u)
                        if not u or not k or k in seen:
                            continue
                        seen.add(k)
                        unique_nodes.append(node)
                    except:
                        try:
                            u = _sanitize_vless_uri(node.get("uri", ""))
                            k = _uri_dedup_key(u)
                            if u and k and k in seen:
                                continue
                            if k:
                                seen.add(k)
                        except:
                            pass
                        unique_nodes.append(node)
                if not unique_nodes:
                    continue
                items.append(Header(text=f"{sub['name']} ({len(unique_nodes)})"))
                for node in unique_nodes:
                    is_act = (node["uri"] == active)
                    ping = node.get("ping", -1)
                    ping_str = f" — {ping}ms" if ping > 0 else ""
                    label = node["name"] + ping_str
                    items.append(Text(text=label, accent=bool(is_act),
                                      on_click=self._make_vless_node_select(node["uri"])))

        if not is_custom:
            items += [
                Text(text=Z("referral_btn"),
                     icon="msg_link",
                     accent=True,
                     on_click=self._make_referral_click(sponsor_idx, "vless")),
                Divider(),
            ]
        items += [
            Text(text=Z("vless_nodes_manage"), icon="msg_settings_14",
                 on_click=lambda v: self._vless_open_nodes_menu()),
            Divider(),
        ]
        return items

    def _add_menu_items(self):
        def on_click(ctx):
            self._open_settings()

        try:
            self.remove_menu_item(self._chat_menu_item_id)
        except:
            pass

        try:
            self.add_menu_item(MenuItemData(
                menu_type=MenuItemType.CHAT_ACTION_MENU,
                item_id=self._chat_menu_item_id,
                text=Z("menu_settings"),
                on_click=on_click,
                icon="msg_settings",
                priority=1,
            ))
        except:
            pass

        try:
            self.remove_menu_item(self._drawer_menu_item_id)
        except:
            pass

        try:
            self.add_menu_item(MenuItemData(
                menu_type=MenuItemType.DRAWER_MENU,
                item_id=self._drawer_menu_item_id,
                text=Z("menu_settings"),
                on_click=on_click,
                icon="msg_settings",
                priority=1,
            ))
        except:
            pass

    def _remove_menu_items(self):
        try:
            self.remove_menu_item(self._chat_menu_item_id)
        except:
            pass
        try:
            self.remove_menu_item(self._drawer_menu_item_id)
        except:
            pass

    def _open_settings(self):
        try:
            PC = find_class("com.exteragram.messenger.plugins.PluginsController")
        except:
            PC = None
        try:
            PSA = find_class("com.exteragram.messenger.plugins.ui.PluginSettingsActivity")
        except:
            PSA = None

        try:
            if PC:
                inst = None
                try:
                    inst = PC.getInstance()
                except:
                    inst = None
                if inst is not None and hasattr(inst, "plugins") and PSA:
                    plugin_obj = None
                    try:
                        plugin_obj = inst.plugins.get(self.id)
                    except:
                        plugin_obj = None
                    if plugin_obj is not None:
                        frag = get_last_fragment()
                        if frag is not None:
                            _log("open_settings: PluginSettingsActivity")
                            frag.presentFragment(PSA(plugin_obj))
                            return
                try:
                    _log("open_settings: PC.openPluginSettings(self.id)")
                    PC.openPluginSettings(self.id)
                    return
                except:
                    try:
                        _log("open_settings: PC.openPluginSettings('exitfy')")
                        PC.openPluginSettings("exitfy")
                        return
                    except:
                        pass
            frag = get_last_fragment()
            if frag and PC:
                _log("open_settings: PluginsController.getInstance().openPluginSettings")
                PC.getInstance().openPluginSettings(self.id, frag)
        except:
            pass

    def _on_mode_changed(self, v):
        prev_mode = _to_int(self.get_setting("mode", 1), 1)
        mode = _to_int(v, prev_mode)
        self._watchdog_stop.set()
        self._vless_disconnect(silent=True)
        try:
            self._tgws_disconnect(silent=True)
        except:
            pass
        if int(prev_mode) != 2:
            _set_tg_proxy_enabled(False)
        if mode == 1:
            self._ping_stop.clear()
            self._health_stop.clear()
            threading.Thread(target=self._vless_pinger_loop, daemon=True).start()
            threading.Thread(target=self._vless_health_check_loop, daemon=True).start()
            if not self._vless_data.get("subs") and not self._vless_init_in_progress:
                threading.Thread(target=self._init_default_vless_sub, daemon=True).start()
            if self.get_setting("vless_enabled", False) and self._vless_active_uri:
                threading.Thread(target=self._vless_connect, daemon=True).start()
        elif mode == 0:
            if self.get_setting("proxy_enabled", False):
                last_link = _pget_str("last_proxy_link", "")
                if last_link:
                    info = _parse_link(last_link)
                    if info:
                        _apply_proxy_info(info)
                _set_tg_proxy_enabled(True)
                self._watchdog_stop.clear()
                threading.Thread(target=self._watchdog_loop, daemon=True).start()
        else:
            try:
                if not bool(self.get_setting("proxy_enabled", False)):
                    self.set_setting("proxy_enabled", True, reload_settings=True)
            except:
                pass
            threading.Thread(target=self._tgws_connect_bg, daemon=True).start()
        self._reload_settings_ui()
        try:
            cnt = 1 if mode == 1 else (2 if mode == 0 else 3)
            self._mkstats_event("mode_changed", count=cnt)
        except:
            pass
        try:
            self._pill_update()
        except:
            pass

    def _on_enabled_changed(self, v):
        enabled = bool(v)
        mode = _to_int(self.get_setting("mode", 1), 1)
        if mode == 1:
            if enabled:
                if self._vless_active_uri:
                    threading.Thread(target=self._vless_connect, daemon=True).start()
            else:
                self._vless_disconnect()
        elif mode == 0:
            if enabled:
                last_link = _pget_str("last_proxy_link", "")
                if last_link:
                    info = _parse_link(last_link)
                    if info:
                        _apply_proxy_info(info)
                _set_tg_proxy_enabled(True)
                self._watchdog_stop.clear()
                threading.Thread(target=self._watchdog_loop, daemon=True).start()
            else:
                self._watchdog_stop.set()
                _set_tg_proxy_enabled(False)
        else:
            if enabled:
                threading.Thread(target=self._tgws_connect_bg, daemon=True).start()
            else:
                self._tgws_disconnect()

        try:
            self._pill_update()
        except:
            pass

    def _on_provider_changed(self, v):
        self._proxy_links = []
        self._proxy_cycle_idx = 0
        try:
            self._mkstats_track("proxy_provider_changed", count=int(v) + 1, cooldown_sec=3)
        except:
            pass
        if self.get_setting("proxy_enabled", False):
            threading.Thread(target=self._proxy_refresh_bg, daemon=True).start()
        self._reload_settings_ui()

    def _on_vless_provider_changed(self, v):
        provider = int(v)
        self._vless_active_uri = ""
        try:
            self._mkstats_track("vless_provider_changed", count=int(provider) + 1, cooldown_sec=3)
        except:
            pass
        if provider == 3:
            self._vless_data["manual"] = []
            self._vless_data["subs"] = []
            self._vless_save_data()
            if self._vless_running:
                self._vless_disconnect(silent=True)
            self._reload_settings_ui()
            return
        try:
            self.set_setting("vless_custom_sub_url", "")
            self._vless_custom_sub_url = ""
        except:
            pass
        self._vless_data["subs"] = []
        self._vless_data["manual"] = []
        self._vless_save_data()
        if self._vless_running:
            self._vless_disconnect(silent=True)
        threading.Thread(
            target=self._init_default_vless_sub, args=(provider,), daemon=True
        ).start()
        self._reload_settings_ui()

    def _on_vless_custom_key_input(self, v):
        uri = str(v or "").strip()
        self._vless_custom_key_buf = uri
        if uri.startswith("vless://") and "@" in uri and uri != self._last_custom_key_applied:
            self._last_custom_key_applied = uri
            self._apply_vless_custom_key()

    def _apply_vless_custom_key(self):
        uri = getattr(self, "_vless_custom_key_buf", "").strip()
        uri = _sanitize_vless_uri(uri)
        if not uri.startswith("vless://") or len(uri) < 20:
            run_on_ui_thread(lambda: BulletinHelper.show_error("Неверный vless:// ключ")); return
        conf, name = VlessConfigParser.parse(uri, 1080)
        if not conf:
            run_on_ui_thread(lambda: BulletinHelper.show_error("Ошибка парсинга ключа")); return
        manual = self._vless_data.get("manual", [])
        k = _uri_dedup_key(uri)
        if not any(_uri_dedup_key(n.get("uri", "")) == k for n in manual):
            manual.append({"name": name, "uri": uri, "ping": -1})
            self._vless_data["manual"] = manual
        self._vless_active_uri = uri
        self._vless_save_data()
        self._ping_event.set()
        try:
            self.set_setting("vless_enabled", True, reload_settings=True)
        except:
            pass
        threading.Thread(target=self._vless_connect, daemon=True).start()
        run_on_ui_thread(self._reload_settings_ui)

    def _on_vless_custom_sub_input(self, v):
        url = str(v or "").strip()
        self._vless_custom_sub_buf = url
        if url.startswith("http") and len(url) > 10 and url != self._last_custom_sub_applied:
            self._last_custom_sub_applied = url
            self._apply_vless_custom_sub()

    def _apply_vless_custom_sub(self):
        url = getattr(self, "_vless_custom_sub_buf", "").strip()
        if not url.startswith("http") or len(url) < 10:
            run_on_ui_thread(lambda: BulletinHelper.show_error(Z("vless_custom_sub_bad")))
            return
        try:
            self.set_setting("vless_custom_sub_url", url)
            self._vless_custom_sub_url = url
        except:
            pass
        threading.Thread(target=self._vless_do_add_sub, args=(url,), daemon=True).start()

    def _on_connect_try_changed(self, v):
        try:
            val = max(1, min(30, int(str(v).strip())))
            _pset_int("proxy_connect_try", val)
        except: pass

    def _on_stuck_changed(self, v):
        try:
            val = max(1, min(120, int(str(v).strip())))
            _pset_int("proxy_stuck_sec", val)
        except: pass

    def _on_cooldown_changed(self, v):
        try:
            val = max(0, min(120, int(str(v).strip())))
            _pset_int("proxy_cooldown_sec", val)
        except: pass

    def _on_auto_switch_changed(self, v):
        auto = bool(v)
        mode = _to_int(self.get_setting("mode", 1), 1)
        if mode != 0:
            self._reload_settings_ui()
            return
        if auto:
            self._watchdog_stop.clear()
            if self.get_setting("proxy_enabled", False):
                threading.Thread(target=self._watchdog_loop, daemon=True).start()
        else:
            self._watchdog_stop.set()
            if self.get_setting("proxy_enabled", False) and not self._proxy_links:
                threading.Thread(target=self._proxy_refresh_bg, daemon=True).start()
        self._reload_settings_ui()

    def _on_manual_node_changed(self, v):
        try:
            idx = int(v)
            links = self._proxy_links or []
            if idx < 0 or idx >= len(links):
                return
            link = links[idx]
            info = _parse_link(link)
            if info:
                _pset_str("last_proxy_link", link)
                _apply_proxy_info(info)
                self._proxy_cycle_idx = idx
        except Exception as e:
            _log(f"manual node change error: {e}")

    def _reload_settings_ui(self):
        def _do():
            try:
                self.set_setting("__reload__", time.time(), reload_settings=True)
            except: pass
        run_on_ui_thread(_do, 100)

    def _proxy_refresh(self):
        threading.Thread(target=self._proxy_refresh_bg, daemon=True).start()

    def _proxy_refresh_bg(self):
        provider = self.get_setting("proxy_provider", 0)
        links = _fetch_proxy_links(provider)
        if not links:
            try:
                self._mkstats_track("proxy_refresh_empty", cooldown_sec=20)
            except:
                pass
            return
        scored = _score_links(links)
        self._proxy_links = [l for _, l in scored]
        try:
            self._mkstats_track("proxy_refresh_ok", cooldown_sec=20)
        except:
            pass
        self._proxy_cycle_idx = 0
        if not self.get_setting("proxy_auto_switch", True):
            best_link = self._proxy_links[0] if self._proxy_links else None
            if best_link:
                info = _parse_link(best_link)
                if info:
                    _pset_str("last_proxy_link", best_link)
                    _apply_proxy_info(info)
            self._reload_settings_ui()
        else:
            best_link = self._proxy_links[0] if self._proxy_links else None
            if best_link:
                info = _parse_link(best_link)
                if info:
                    _pset_str("last_proxy_link", best_link)
                    _apply_proxy_info(info)

    def _open_tg_proxy_settings(self):
        try:
            frag = get_last_fragment()
            if frag:
                from org.telegram.ui import ProxyListActivity
                frag.presentFragment(ProxyListActivity())
        except: pass

    def _open_tg_link(self, url):
        try:
            url = str(url or "").strip()
            if not url: return
            _log(f"open link: {url}")
            try:
                frag = get_last_fragment()
                act = frag.getParentActivity() if frag else None
            except:
                act = None

            try:
                from org.telegram.messenger.browser import Browser
            except:
                Browser = None

            ctx = act if act else ApplicationLoader.applicationContext
            if Browser:
                _log(f"open link via Browser.openUrl (ctx={type(ctx)})")
                Browser.openUrl(ctx, url)
                return

            from android.content import Intent
            from android.net import Uri
            intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
            if not act:
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            ctx.startActivity(intent)
        except:
            pass

    def _make_referral_click(self, provider_idx, source):
        def handler(v):
            try:
                url = REFERRAL.get(provider_idx, "")
                _log(f"referral click ({source}): idx={provider_idx} url={url}")
                self._open_tg_link(url)
            except Exception as e:
                _log(f"referral click error ({source}): {e}")
        return handler


    class _ExitFyPillClickHook(MethodHook):
        def __init__(self, plugin):
            super().__init__()
            self.plugin = plugin
        def before_hooked_method(self, param):
            try:
                if not self.plugin._pill_is_ours(param.thisObject):
                    return
                self.plugin._pill_toggle()
                try:
                    param.setResult(None)
                except Exception:
                    pass
            except Exception:
                pass


    class _ExitFyPillLongClickHook(MethodHook):
        def __init__(self, plugin):
            super().__init__()
            self.plugin = plugin
        def before_hooked_method(self, param):
            try:
                if not self.plugin._pill_is_ours(param.thisObject):
                    return
                self.plugin._pill_open_settings()
                try:
                    param.setResult(True)
                except Exception:
                    pass
            except Exception:
                pass


    class _ExitFyPillUpdateHook(MethodHook):
        def __init__(self, plugin):
            super().__init__()
            self.plugin = plugin
        def after_hooked_method(self, param):
            try:
                if not self.plugin._pill_is_ours(param.thisObject):
                    return
                try:
                    self.plugin._pill_apply_visuals_standalone(param.thisObject)
                except Exception:
                    pass
                try:
                    self.plugin._pill_update_standalone()
                except Exception:
                    pass
            except Exception:
                pass


    class _ExitFyPillAttachHook(MethodHook):
        def __init__(self, plugin):
            super().__init__()
            self.plugin = plugin
        def after_hooked_method(self, param):
            try:
                if not self.plugin._pill_is_ours(param.thisObject):
                    return
                try:
                    self.plugin._pill_remember_instance(param.thisObject)
                except Exception:
                    pass
                try:
                    self.plugin._pill_apply_visuals_standalone(param.thisObject)
                except Exception:
                    pass
            except Exception:
                pass

    def _open_referral(self, provider_idx):
        try:
            url = REFERRAL.get(provider_idx, "")
            self._open_tg_link(url)
        except: pass

    def _make_proxy_sub_delete(self, url):
        def handler(v):
            if url in self._proxy_custom_urls:
                self._proxy_custom_urls.remove(url)
                _pset_str("proxy_custom_urls", json.dumps(self._proxy_custom_urls))
            self._reload_settings_ui()
        return handler

    def _watchdog_loop(self):
        connect_try = _pget_int("proxy_connect_try", 7)
        stuck_sec = _pget_int("proxy_stuck_sec", 8)
        cooldown_sec = _pget_int("proxy_cooldown_sec", 5)

        while not self._watchdog_stop.is_set():
            try:
                if not self.get_setting("proxy_enabled", False):
                    self._watchdog_stop.wait(CHECK_INTERVAL_SEC); continue
                if not self.get_setting("proxy_auto_switch", True):
                    self._watchdog_stop.wait(CHECK_INTERVAL_SEC); continue

                if not self._proxy_links:
                    provider = self.get_setting("proxy_provider", 0)
                    links = _fetch_proxy_links(provider)
                    if links:
                        scored = _score_links(links)
                        self._proxy_links = [l for _, l in scored]
                        self._proxy_cycle_idx = 0
                    else:
                        self._watchdog_stop.wait(10); continue

                connected = self._wait_connected(connect_try)
                if connected:
                    self._watchdog_stop.wait(stuck_sec)
                    if self._watchdog_stop.is_set(): break
                    still_ok = self._wait_connected(3)
                    if still_ok:
                        self._watchdog_stop.wait(CHECK_INTERVAL_SEC); continue

                self._switch_proxy()
                if cooldown_sec > 0:
                    self._watchdog_stop.wait(cooldown_sec)
            except:
                self._watchdog_stop.wait(CHECK_INTERVAL_SEC)

    def _wait_connected(self, timeout_sec):
        deadline = time.time() + timeout_sec
        while time.time() < deadline:
            try:
                state = int(ConnectionsManager.getInstance(0).getConnectionState())
                if state == 3:
                    return True
            except: pass
            time.sleep(0.5)
        return False

    def _switch_proxy(self):
        if not self._proxy_links: return
        if random.random() < RANDOM_PROXY_CHANCE:
            idx = random.randint(0, len(self._proxy_links) - 1)
        else:
            idx = self._proxy_cycle_idx % len(self._proxy_links)
            self._proxy_cycle_idx += 1

        link = self._proxy_links[idx]
        info = _parse_link(link)
        if info:
            _pset_str("last_proxy_link", link)
            _apply_proxy_info(info)

    def _vless_save_data(self):
        self._vless_data["active_uri"] = self._vless_active_uri

        try:
            def _dedup_nodes(nodes):
                out = []
                seen = set()
                for n in (nodes or []):
                    try:
                        u0 = _sanitize_vless_uri(n.get("uri", ""))
                        k = _uri_dedup_key(u0)
                        if not u0 or not k:
                            continue
                        if k in seen:
                            continue
                        seen.add(k)
                        nn = dict(n)
                        nn["uri"] = u0
                        out.append(nn)
                    except:
                        continue
                return out

            self._vless_data["manual"] = _dedup_nodes(self._vless_data.get("manual", []))
            subs = []
            for sub in (self._vless_data.get("subs", []) or []):
                try:
                    s2 = dict(sub)
                    s2["nodes"] = _dedup_nodes(sub.get("nodes", []))
                    subs.append(s2)
                except:
                    subs.append(sub)
            self._vless_data["subs"] = subs
        except:
            pass

        self.set_setting("vless_data", self._vless_data)

    def _vless_connect(self):
        if not self._vless_active_uri: return
        self._vless_disconnect(silent=True)
        if not self._vless_core.is_available():
            try:
                self._mkstats_track("vless_connect_core_unavailable", cooldown_sec=10)
            except:
                pass
            return
        port = _get_free_port()
        self._vless_port = port
        config_json, _ = VlessConfigParser.parse(self._vless_active_uri, port)
        if not config_json:
            try:
                self._mkstats_track("vless_connect_parse_fail", cooldown_sec=10)
            except:
                pass
            return
        _log(f"VLESS starting on port {port}")
        try:
            self._mkstats_track("vless_connect_start", cooldown_sec=5)
        except:
            pass
        err = self._vless_core.start(config_json)
        if err:
            _log(f"VLESS core error: {err}")
            try:
                self._mkstats_track("vless_connect_core_error", cooldown_sec=10)
            except:
                pass
            return

        def connection_watchdog():
            for _ in range(20):
                if _check_vless_port(port, timeout=0.05):
                    _vless_apply_tg_proxy(port, True)
                    self._vless_running = True
                    try:
                        self.set_setting("vless_enabled", True, reload_settings=True)
                    except:
                        pass
                    try:
                        self._pill_update()
                    except:
                        pass
                    try:
                        self._mkstats_track("vless_connect_ok", cooldown_sec=5)
                    except:
                        pass
                    run_on_ui_thread(self._reload_settings_ui)
                    return
                time.sleep(0.3)
            _log("VLESS port timeout")
            try:
                self._mkstats_track("vless_connect_timeout", cooldown_sec=10)
            except:
                pass
            run_on_ui_thread(lambda: self._vless_disconnect(silent=True))
        threading.Thread(target=connection_watchdog, daemon=True).start()

    def _vless_disconnect(self, silent=False):
        self._vless_core.stop()
        self._vless_running = False
        _vless_apply_tg_proxy(0, False)
        if not silent:
            self._reload_settings_ui()

    def _tgws_proxy_link(self):
        try:
            port = int(self._tgws_port or 0)
        except:
            port = 0
        if port <= 0:
            port = int(TGWS_LOCAL_PORT)
        return f"tg://socks?server={VLESS_LOCAL_HOST}&port={int(port)}"

    def _tgws_connect(self):
        try:
            try:
                self._tgws_last_error = ""
            except Exception:
                pass
            try:
                self._vless_disconnect(silent=True)
            except:
                pass
            port = _get_free_port()
            if int(port or 0) <= 0:
                try:
                    self._tgws_last_error = "no free port"
                except Exception:
                    pass
                self._tgws_running = False
                self._tgws_port = 0
                return False
            self._tgws_port = int(port)
            config_json = json.dumps({"port": int(port), "verbose": False})
            err = self._tgws_core.start(config_json)
            if err:
                try:
                    self._tgws_last_error = str(err)
                except Exception:
                    pass
                try:
                    _log(f"TGWS start error: {err}")
                except:
                    pass
                try:
                    run_on_ui_thread(lambda: BulletinHelper.show_error(str(err)))
                except:
                    pass
                self._tgws_running = False
                self._tgws_port = 0
                return False

            def _watchdog():
                deadline = time.time() + float(TGWS_LOCAL_READY_TIMEOUT_SEC)
                while time.time() < deadline:
                    if _check_vless_port(int(port), timeout=0.5):
                        try:
                            self._tgws_snapshot_current_proxy()
                        except Exception:
                            pass
                        try:
                            self._tgws_proxy_managed = True
                        except Exception:
                            pass
                        _vless_apply_tg_proxy(int(port), True)
                        self._tgws_running = True
                        try:
                            self.set_setting("proxy_enabled", True, reload_settings=True)
                        except:
                            pass
                        try:
                            self._pill_update()
                        except:
                            pass
                        try:
                            run_on_ui_thread(self._reload_settings_ui)
                        except:
                            pass
                        try:
                            BulletinHelper.show_success("TG WS подключен")
                        except:
                            pass
                        return
                    time.sleep(0.3)
                self._tgws_running = False
                self._tgws_port = 0
                try:
                    _log("TGWS local port timeout")
                except:
                    pass
                try:
                    self._tgws_last_error = "timeout"
                except Exception:
                    pass
                try:
                    run_on_ui_thread(lambda: BulletinHelper.show_error("TG WS: timeout"))
                except:
                    pass
                try:
                    self.set_setting("proxy_enabled", False, reload_settings=True)
                except:
                    pass
                try:
                    self._pill_update()
                except:
                    pass
                try:
                    self._tgws_core.stop()
                except:
                    pass

            threading.Thread(target=_watchdog, daemon=True).start()
            return True
        except:
            self._tgws_running = False
            self._tgws_port = 0
            try:
                self._tgws_last_error = "exception"
            except Exception:
                pass
            try:
                self._tgws_core.stop()
            except:
                pass
            return False

    def _tgws_disconnect_with_reason(self, reason, silent=False):
        self._tgws_disconnect(silent=bool(silent))

    def _tgws_disconnect(self, silent=False):
        try:
            self._tgws_core.stop()
        except:
            pass
        self._tgws_running = False
        self._tgws_port = 0
        try:
            self._tgws_restore_previous_proxy()
        except Exception:
            _vless_apply_tg_proxy(0, False)
        try:
            self._pill_update()
        except:
            pass
        if not silent:
            self._reload_settings_ui()

    def _make_vless_node_select(self, uri):
        def handler(v):
            prev = self._vless_active_uri
            self._vless_active_uri = uri
            self._vless_save_data()
            try:
                self._mkstats_track("vless_node_selected", cooldown_sec=2)
            except:
                pass
            if prev == uri and self._vless_running:
                try:
                    self.set_setting("vless_enabled", False, reload_settings=True)
                except:
                    pass
                self._vless_disconnect()
            else:
                try:
                    self.set_setting("vless_enabled", True, reload_settings=True)
                except:
                    pass
                threading.Thread(target=self._vless_connect, daemon=True).start()
            self._reload_settings_ui()
        return handler

    def _vless_open_nodes_menu(self):
        frag = get_last_fragment()
        if not frag: return
        builder = AlertDialogBuilder(frag.getParentActivity())
        items = [Z("vless_nodes_update"), Z("vless_nodes_clear")]

        cancel_text = Z("btn_cancel")
        builder.set_negative_button(cancel_text)

        def on_item(dlg, which):
            try:
                if int(which) == 0:
                    threading.Thread(target=self._vless_update_all_subs, daemon=True).start()
                elif int(which) == 1:
                    self._vless_clear_user_nodes()
            except Exception as e:
                _log(f"nodes menu error: {e}")

        builder.set_title(Z("vless_nodes_manage"))
        builder.set_items(items, on_item)
        builder.set_negative_button(cancel_text, lambda d, w: d.dismiss())
        builder.show()

    def _vless_update_all_subs(self):
        urls = [s.get("url", "") for s in self._vless_data.get("subs", []) if s.get("url")]
        try:
            if _to_int(self.get_setting("vless_provider", 0), 0) == 3:
                u = str(self.get_setting("vless_custom_sub_url", "") or "").strip()
                if u and u not in urls:
                    urls.insert(0, u)
        except:
            pass
        for url in urls:
            self._vless_update_sub(url)

    def _vless_clear_user_nodes(self):
        self._vless_data["manual"] = []
        self._vless_data["subs"] = []
        if self._vless_active_uri:
            self._vless_active_uri = ""
            self._vless_disconnect(silent=True)
        self._vless_save_data()
        self._reload_settings_ui()

    def _vless_do_add_sub(self, url):
        if not url.startswith("http"): return
        nodes = _fetch_vless_nodes(url)
        if not nodes:
            def _notify():
                try:
                    try:
                        r = requests.get(url, timeout=15)
                        t = (r.text or "").strip()
                    except:
                        t = ""
                    low = t.lower()
                    if "приложение не поддерживается" in low or "not supported" in low:
                        BulletinHelper.show_error(Z("vless_app_not_supported"))
                    else:
                        BulletinHelper.show_error(Z("vless_custom_sub_empty"))
                except:
                    BulletinHelper.show_error(Z("vless_custom_sub_empty"))

            run_on_ui_thread(_notify)
            return
        if nodes:
            if "subs" not in self._vless_data: self._vless_data["subs"] = []
            existing = [s["url"] for s in self._vless_data["subs"]]
            if url not in existing:
                self._vless_data["subs"].append({
                    "id": url,
                    "url": url,
                    "name": urllib.parse.urlparse(url).netloc,
                    "nodes": nodes
                })
            self._vless_save_data()
            self._ping_event.set()
            run_on_ui_thread(self._reload_settings_ui)

    def _vless_update_sub(self, url):
        provider = _to_int(self.get_setting("vless_provider", 0), 0)
        nodes = _fetch_vless_nodes(url)
        changed = False
        for sub in self._vless_data.get("subs", []):
            if sub["url"] == url and nodes:
                sub["nodes"] = nodes
                changed = True
        if not changed and provider == 3 and nodes:
            try:
                if "subs" not in self._vless_data:
                    self._vless_data["subs"] = []
                existing = [s.get("url", "") for s in self._vless_data.get("subs", [])]
                if url not in existing:
                    self._vless_data["subs"].append({
                        "id": url,
                        "url": url,
                        "name": urllib.parse.urlparse(url).netloc,
                        "nodes": nodes
                    })
                    changed = True
            except:
                pass
        if changed:
            self._vless_save_data()
            self._ping_event.set()
            run_on_ui_thread(self._reload_settings_ui)

    def _vless_pinger_loop(self):
        while not self._ping_stop.is_set():
            self._ping_event.clear()
            try:
                all_nodes = list(self._vless_data.get("manual", []))
                for sub in self._vless_data.get("subs", []):
                    all_nodes.extend(sub.get("nodes", []))
                if not all_nodes:
                    self._ping_event.wait(600); continue
                changed = False
                for node in all_nodes:
                    if self._ping_stop.is_set(): break
                    try:
                        u = _sanitize_vless_uri(node.get("uri", ""))
                        host = None
                        port = None
                        if u.startswith("vless://") or u.startswith("trojan://"):
                            r = VlessConfigParser._parse_std_uri(u)
                            if r:
                                host, port = r.get("host"), r.get("port")
                        elif u.startswith("ss://"):
                            body = u[5:].split("#", 1)[0]
                            if "@" in body:
                                server_part = body.split("@", 1)[1].split("?", 1)[0].strip().rstrip("/")
                                if ":" in server_part:
                                    host, port = server_part.rsplit(":", 1)
                            else:
                                try:
                                    b64 = body.replace("-", "+").replace("_", "/")
                                    miss = len(b64) % 4
                                    if miss:
                                        b64 += "=" * (4 - miss)
                                    decoded = base64.b64decode(b64, validate=False).decode("utf-8", errors="ignore")
                                    if "@" in decoded and ":" in decoded:
                                        server_part = decoded.split("@", 1)[1]
                                        if ":" in server_part:
                                            host, port = server_part.rsplit(":", 1)
                                except:
                                    pass
                        elif u.startswith("vmess://"):
                            try:
                                b64_part = u[8:].split("#", 1)[0].strip().replace("-", "+").replace("_", "/")
                                miss = len(b64_part) % 4
                                if miss:
                                    b64_part += "=" * (4 - miss)
                                raw = base64.b64decode(b64_part, validate=False)
                                obj = json.loads(raw.decode("utf-8", errors="ignore"))
                                host = obj.get("add")
                                port = obj.get("port")
                            except:
                                pass
                        if not host or not port:
                            continue
                        start = time.time()
                        s = socket.create_connection((host, int(port)), timeout=1.5)
                        p = int((time.time() - start) * 1000); s.close()
                        if node.get("ping") != p: node["ping"] = p; changed = True
                    except:
                        if node.get("ping") != 9999: node["ping"] = 9999; changed = True
                if changed: self._vless_save_data()
            except: pass
            self._ping_event.wait(600)

    def _vless_health_check_loop(self):
        while not self._health_stop.is_set():
            self._health_stop.wait(VLESS_HEALTH_INTERVAL_SEC)
            if self._health_stop.is_set():
                break
            try:
                mode = _to_int(self.get_setting("mode", 1), 1)
                if mode != 1:
                    self._health_fail_count = 0
                    continue
                if not self.get_setting("proxy_auto_switch", True):
                    self._health_fail_count = 0
                    continue
                if not self.get_setting("vless_enabled", False):
                    self._health_fail_count = 0
                    continue
                if not self._vless_running:
                    self._health_fail_count = 0
                    continue
                port = self._vless_port
                if not port:
                    continue
                alive = _check_vless_port(int(port), timeout=0.3)
                if alive:
                    self._health_fail_count = 0
                    continue
                self._health_fail_count += 1
                if self._health_fail_count >= VLESS_HEALTH_FAIL_THRESHOLD:
                    self._health_fail_count = 0
                    self._vless_auto_switch_to_best()
            except:
                pass

    def _vless_auto_switch_to_best(self):
        try:
            try:
                self._mkstats_track("vless_auto_switch", cooldown_sec=10)
            except:
                pass
            all_nodes = list(self._vless_data.get("manual", []))
            for sub in self._vless_data.get("subs", []):
                all_nodes.extend(sub.get("nodes", []))
            if not all_nodes:
                return
            best = None
            best_ping = 999999
            for n in all_nodes:
                try:
                    p = int(n.get("ping", 999999))
                except:
                    p = 999999
                if p <= 0 or p >= 9999:
                    continue
                if p < best_ping:
                    best_ping = p
                    best = n
            if not best:
                return
            uri = str(best.get("uri", ""))
            if not uri:
                return
            if uri == self._vless_active_uri:
                return
            self._vless_active_uri = uri
            self._vless_data["active_uri"] = uri
            self._vless_save_data()
            if self.get_setting("vless_enabled", False):
                threading.Thread(target=self._vless_connect, daemon=True).start()
                run_on_ui_thread(self._reload_settings_ui)
        except:
            pass

    def _install_clipboard_hook(self):
        try:
            plugin_ref = self
            class _ClipboardHook(MethodHook):
                def after_hooked_method(self, param):
                    try:
                        uris = plugin_ref._get_clipboard_proxy_uris()
                        if not uris:
                            return
                        if uris[0] == getattr(plugin_ref, "_last_clipboard_uri", ""):
                            return
                        manual = plugin_ref._vless_data.get("manual", [])
                        existing = set([_uri_dedup_key(str(n.get("uri", ""))) for n in manual])
                        new_uris = []
                        for u in uris:
                            k = _uri_dedup_key(u)
                            if not k or k in existing:
                                continue
                            new_uris.append(u)
                        plugin_ref._last_clipboard_uri = uris[0]
                        if not new_uris:
                            return
                        plugin_ref._show_clipboard_add_dialog(new_uris)
                    except:
                        pass

            LaunchActivity = find_class("org.telegram.ui.LaunchActivity")
            if LaunchActivity:
                for m in LaunchActivity.getDeclaredMethods():
                    if m.getName() == "onResume" and len(m.getParameterTypes()) == 0:
                        self.hook_method(m, _ClipboardHook())
                        break
        except:
            pass

    def _get_clipboard_proxy_uris(self):
        try:
            ctx = ApplicationLoader.applicationContext
            cm = ctx.getSystemService("clipboard")
            if not cm or not cm.hasPrimaryClip():
                return []
            clip = cm.getPrimaryClip()
            if not clip or clip.getItemCount() == 0:
                return []
            item = clip.getItemAt(0)
            if not item:
                return []
            text = str(item.getText() or "")
            if not text:
                return []
            found = []
            for scheme in _PROXY_SCHEMES:
                try:
                    found.extend(re.findall(re.escape(scheme) + r"[^\s]+", text))
                except:
                    continue
            out = []
            for u in found:
                su = _sanitize_vless_uri(u)
                if su and _is_proxy_uri(su):
                    out.append(su)
            return out[:10]
        except:
            return []

    def _show_clipboard_add_dialog(self, uris_or_uri):
        if isinstance(uris_or_uri, str):
            uris = [uris_or_uri]
        else:
            uris = list(uris_or_uri or [])
        if not uris:
            return
        def _do():
            try:
                b = AlertDialogBuilder(get_last_fragment())
                if len(uris) == 1:
                    cfg, name = VlessConfigParser.parse(uris[0], 1080)
                    if not name:
                        name = Z("vless_node_default")
                    short = uris[0][:60] + "..." if len(uris[0]) > 60 else uris[0]
                    msg = f"{name}\n\n{short}"
                    title = Z("vless_add_key")
                else:
                    title = Z("vless_add_key")
                    lines = []
                    for u in uris[:10]:
                        _, n = VlessConfigParser.parse(u, 1080)
                        n = n or Z("vless_node_default")
                        lines.append("- " + n)
                    msg = "\n".join(lines)

                b.set_title(str(title))
                b.set_message(str(msg))
                def add_all(d=None, w=None):
                    try:
                        added = 0
                        manual = self._vless_data.get("manual", [])
                        existing = set([_uri_dedup_key(str(n.get("uri", ""))) for n in manual])
                        for u in uris:
                            k = _uri_dedup_key(u)
                            if not k or k in existing:
                                continue
                            cfg, name = VlessConfigParser.parse(u, 1080)
                            if not cfg:
                                continue
                            manual.append({"name": name or Z("vless_node_default"), "uri": u, "ping": -1})
                            existing.add(k)
                            added += 1
                        self._vless_data["manual"] = manual
                        self._vless_save_data()
                        self._ping_event.set()
                        run_on_ui_thread(self._reload_settings_ui)
                        if added > 0:
                            BulletinHelper.show_success(Z("refresh"))
                    except:
                        pass
                    try:
                        if d: d.dismiss()
                    except:
                        pass
                b.set_positive_button(Z("btn_add") if "btn_add" in TRANSLATIONS else "OK", add_all)
                b.set_negative_button(Z("btn_cancel"), lambda d, w: d.dismiss())
                b.show()
            except:
                pass
        run_on_ui_thread(_do)
